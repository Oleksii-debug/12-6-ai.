...
def _cleanup_worker(
    ...
    started.set()
    result_queue.put(cleanup_recovery_generations(root, keep=1))
    done.set()
...
def test_cleanup_waits_for_active_publisher_and_preserves_new_current(tmp_path: Path) -> None:
    model, trainer, cfg = _stack()
    root = tmp_path / "recovery"
    _step(trainer)
    _publish(root, model, trainer, cfg)
    ...
    ctx = multiprocessing.get_context("spawn")
    publisher_entered = ctx.Event()
    publisher_release = ctx.Event()
    publisher_results = ctx.Queue()
    cleanup_started = ctx.Event()
    cleanup_done = ctx.Event()
    cleanup_results = ctx.Queue()

    publisher = ctx.Process(
        target=_concurrent_publish_worker,
        args=(str(root), 2, publisher_entered, publisher_release, publisher_results),
    )
    cleanup = ctx.Process(
        target=_cleanup_worker,
        args=(str(root), cleanup_started, cleanup_done, cleanup_results),
    )
    publisher.start()
    assert publisher_entered.wait(20), "publisher did not reach locked save boundary"
    cleanup.start()
    assert cleanup_started.wait(20), "cleanup process did not reach call boundary"
    assert not cleanup_done.wait(0.75), "cleanup completed while publisher still held lock"
    assert (root / "generations" / "generation-00000001").is_dir()

    publisher_release.set()
    assert cleanup_done.wait(20), "cleanup did not proceed after publisher released lock"
    publisher.join(30)
    cleanup.join(30)
    assert publisher.exitcode == 0
    assert cleanup.exitcode == 0

    status, second = publisher_results.get(timeout=5)
    cleanup_result = cleanup_results.get(timeout=5)
    assert status == "ok"
    assert second["generation"] == 2
    assert cleanup_result["current_generation"] == 2
    assert cleanup_result["retained_generation_count"] == 1
    assert not (root / "generations" / "generation-00000001").exists()
