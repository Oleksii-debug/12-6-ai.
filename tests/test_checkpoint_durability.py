...
def test_fsync_parent_directory_rejects_symlink_parent(tmp_path: Path) -> None:
...
def test_installed_save_orders_tree_fsync_rename_parent_fsync(
    ...
    monkeypatch.setattr(
        "twelve_six.checkpoint.durability.fsync_checkpoint_tree", tree_barrier
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "twelve_six.checkpoint.durability.fsync_parent_directory", parent_barrier
    )
...
def test_installed_save_rejects_missing_parent_without_creating_ancestors(
    tmp_path: Path,
) -> None:
    core = _fake_core([])
    install(core)
    missing_parent = tmp_path / "missing" / "nested"
    destination = missing_parent / "final"
...
def test_installed_save_rejects_symlink_parent_without_external_write(
    tmp_path: Path,
) -> None:
    core = _fake_core([])
    install(core)
    ...
    with pytest.raises(OSError, match="real non-symlink directory"):
        core.save_checkpoint(linked_parent / "final")
...
def test_tree_fsync_failure_never_publishes_destination(
    ...
    monkeypatch.setattr(
        "twelve_six.checkpoint.durability.fsync_checkpoint_tree", fail_tree
    )
    destination = tmp_path / "final"
...
def test_parent_fsync_failure_reports_uncertain_durability_after_atomic_visibility(
    ...
    monkeypatch.setattr(
        "twelve_six.checkpoint.durability.fsync_checkpoint_tree",
        lambda *a, **k: None,
    )
    monkeypatch.setattr(
        "twelve_six.checkpoint.durability.fsync_parent_directory", fail_parent
    )
    destination = tmp_path / "final"
...
def test_destination_collision_after_staging_fails_without_overwrite(
    ...
    monkeypatch.setattr(
        "twelve_six.checkpoint.durability.fsync_checkpoint_tree", inject_collision
    )
