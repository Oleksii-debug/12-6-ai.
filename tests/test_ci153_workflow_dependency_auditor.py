...
""",
    ...
    row = audit_repository(repo)["workflows"][0]
...
def test_data_and_transformer_modules_are_dependency_audited(tmp_path: Path) -> None:
    repo = _repo(tmp_path)
    _write(
        repo / ".github/workflows/data.yml",
        """name: data
jobs:
  test:
    steps:
      - run: python -m datasets.cli.test
      - run: python -m transformers.commands.env
""",
    )
    row = audit_repository(repo)["workflows"][0]
    assert row["classification"] == "MISSING_DECLARED_DEPENDENCY"
    assert row["missing_packages"] == ["datasets", "transformers"]
