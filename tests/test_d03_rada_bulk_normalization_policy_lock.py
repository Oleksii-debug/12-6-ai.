...
def test_normalization_name_and_legacy_decode_are_exact() -> None:
    mutated = copy.deepcopy(CONFIG)
    mutated["normalization"]["name"] = "RADA_VISIBLE_TEXT_HTML_NFKC_V1"
    with pytest.raises(PolicyLockError, match="normalization name drift"):
        validate_policy_lock(mutated)

    mutated = copy.deepcopy(CONFIG)
    mutated["normalization"]["legacy_fallback_encoding"] = "iso-8859-5"
    with pytest.raises(PolicyLockError, match="legacy fallback drift"):
        validate_policy_lock(mutated)
...
def test_output_contract_must_bind_source_encoding_and_hashes() -> None:
    mutated = copy.deepcopy(CONFIG)
    mutated["output_contract"]["jsonl_record_fields"].remove("source_encoding")
...
def test_downstream_decontamination_cannot_be_dropped() -> None:
    mutated = copy.deepcopy(CONFIG)
