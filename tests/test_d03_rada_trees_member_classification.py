...
def test_parent_binding_mutation_rejected() -> None:
    value = copy.deepcopy(config())
    value["parent"]["inventory_implementation_head_sha"] = "0" * 40
...
def test_nul_and_markup_payloads_are_held() -> None:
    policy = config()["classification_policy"]
    assert tool.classify_content("plain/binary.txt", b"text\x00more", policy)["class"] == "BINARY_OR_NUL_HOLD"
    assert (
...
def test_report_verifier_rejects_training_promotion() -> None:
    report = {
        ...
        "worker_id": "D03-RADA-TREES-MEMBER-CLASSIFICATION-20260907",
