...
def test_terminal_authority():
    ...
    assert result["final_test_records"] == 1
    assert result["families"] == 4

def test_materializer_never_reads_final_payload():
    root = Path(__file__).resolve().parents[1]
    rows = materialize_selection_validation(root)
    assert all(r["purpose"] == "selection_validation" for r in rows)
    assert all(r["membership_id"] != "pytest-capture-doc:5de89bc0e3fbd1a3" for r in rows)
...
def test_final_manifest_has_no_plaintext():
    root = Path(__file__).resolve().parents[1]
