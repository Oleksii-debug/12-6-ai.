...
def test_exact_terminal_vector_rejects_recomputed_cpython_capacity_drift() -> None:
    ...
    report["source_vector"]["source_capacity_bytes_before_global_dedup"] = 2215614
    report["source_vector"]["pre_dedup_planning_gap_bytes"] = 17784386
...
def test_exact_terminal_vector_rejects_full_cpython_source_credit() -> None:
    ...
    report["source_vector"]["source_capacity_bytes_before_global_dedup"] = 2217976
    report["source_vector"]["pre_dedup_planning_gap_bytes"] = 17782024
    with pytest.raises(V6PlanningVectorError, match="accepted-only CPython capacity drift"):
        verify_exact_terminal_vector(report)


def test_exact_terminal_vector_rejects_attrs_capacity_omission() -> None:
    report = copy.deepcopy(_report())
    report["source_vector"]["source_capacity_bytes_before_global_dedup"] = 2045180
    report["source_vector"]["source_capacity_by_modality_before_global_dedup"]["code"] = 106031
    report["source_vector"]["pre_dedup_planning_gap_bytes"] = 17954820
    with pytest.raises(V6PlanningVectorError, match="pre-dedup total drift"):
        verify_exact_terminal_vector(report)
