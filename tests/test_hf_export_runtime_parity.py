...
def test_real_trained_s0_export_has_exact_logit_token_decode_parity(tmp_path: Path):
    ...
    assert evidence["truth_boundary"]["promotion_authority"] is False
    assert evidence["truth_boundary"]["transformers_runtime_claimed"] is False
    assert len(evidence["evidence_sha256"]) == 64
    assert len(evidence["prompt_sha256"]) == 3


def test_consumed_export_bytes_are_rehashed_after_v2_verification(tmp_path: Path):
    checkpoint, export = _trained_s0_checkpoint(tmp_path)
...
def test_export_parity_fails_closed_on_verified_export_identity_drift(tmp_path: Path):
    checkpoint, export = _trained_s0_checkpoint(tmp_path)
...
def test_export_parity_cli_emits_machine_json_without_prompt_text(
    ...
    prompt = "private-ish prompt fixture"
    code = main(
        [
            "--checkpoint",
            str(checkpoint),
