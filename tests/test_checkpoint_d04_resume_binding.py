...
def test_binding_rejects_conflicting_existing_resume_schema() -> None:
    manifest = _run_manifest()
    raw = CheckpointIdentity(
        git_sha="a" * 40,
        model_spec={"kind": "schema-conflict-probe"},
        parameter_count=1,
        tokenizer_hash="b" * 64,
        tokenizer_vocab_hash="c" * 64,
        dataset_manifest_hash="d" * 64,
        run_manifest_hash=hash_json(manifest),
        training_config={
            "data": {"resume_binding_schema": "12-6.d04-checkpoint-resume-binding.v2"}
        },
        seed=17,
        precision="fp32",
        step=0,
    ...
    with pytest.raises(
        CheckpointCompatibilityError,
        match="resume_binding_schema conflicts",
    ):
        bind_d04_resume_identity(raw, run_manifest=manifest)


def test_assert_validates_all_actual_d04_hashes_not_only_requested_field() -> None:
    data = {
        "resume_binding_schema": D04_RESUME_BINDING_SCHEMA,
        **_BINDING,
    }
    data["ledger_identity_sha256"] = "g" * 64
    manifest = {"identity": {"training_config": {"data": data}}}

    with pytest.raises(
        CheckpointCompatibilityError,
        match="checkpoint.data.ledger_identity_sha256",
    ):
        assert_d04_resume_binding(
            manifest,
            expected_ordered_next_exposure_identity_sha256=_BINDING[
                "ordered_next_exposure_identity_sha256"
            ],
        )


def test_legacy_checkpoint_without_d04_markers_remains_compatible() -> None:
    manifest = {
        "identity": {
            "training_config": {
                "data": {
    ...
    assert_d04_resume_binding(manifest)


def test_partial_latent_d04_binding_fails_without_caller_expectations() -> None:
    manifest = {
        "identity": {
            "training_config": {
                "data": {"ledger_identity_sha256": _BINDING["ledger_identity_sha256"]}
            }
        }
    }

    with pytest.raises(CheckpointCompatibilityError, match="binding is incomplete"):
        assert_d04_resume_binding(manifest)


def test_malformed_complete_d04_binding_fails_without_caller_expectations() -> None:
    data = {
        "resume_binding_schema": D04_RESUME_BINDING_SCHEMA,
        **_BINDING,
    }
...
def test_complete_d04_binding_validates_without_caller_expectations() -> None:
    data = {
        "resume_binding_schema": D04_RESUME_BINDING_SCHEMA,
        **_BINDING,
    }
