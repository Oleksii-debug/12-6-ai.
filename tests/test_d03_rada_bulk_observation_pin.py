...
from tools.pin_d03_rada_bulk_observation import (
    CONFIG_SCHEMA,
    DERIVED_ARCHIVE_GATE,
    ObservationPinError,
...
def _fixture() -> tuple[bytes, dict, bytes, dict]:
    ...
    probe = {
        ...
        "observation_mode": "CURRENT_MUTABLE_UPSTREAM_ZERO_CREDIT",
        "safe_result": (
...
def test_pin_preserves_failed_capacity_and_zero_credit() -> None:
    archive, probe, probe_bytes, config = _fixture()
    result = pin_observation(archive, probe, probe_bytes, config)
    assert result["gates"]["exact_archive_identity"] == DERIVED_ARCHIVE_GATE
...
def test_pin_is_deterministic() -> None:
    archive, probe, probe_bytes, config = _fixture()
    first = pin_observation(archive, probe, probe_bytes, config)
    second = pin_observation(archive, probe, probe_bytes, config)
    assert _canonical_json_bytes(first) == _canonical_json_bytes(second)


def test_report_sha_tamper_fails() -> None:
    archive, probe, probe_bytes, config = _fixture()
    config["parent_probe"]["probe_report_sha256"] = "0" * 64
    with pytest.raises(ObservationPinError):
        pin_observation(archive, probe, probe_bytes, config)


def test_archive_tamper_fails() -> None:
    archive, probe, probe_bytes, config = _fixture()
    with pytest.raises(ObservationPinError):
        pin_observation(archive + b"x", probe, probe_bytes, config)


def test_inventory_identity_tamper_fails() -> None:
...
def test_capacity_promotion_attack_fails() -> None:
    archive, probe, _, config = _fixture()
    probe = copy.deepcopy(probe)
    config = copy.deepcopy(config)
...
def test_training_credit_attack_fails() -> None:
    archive, probe, _, config = _fixture()
    probe = copy.deepcopy(probe)
    config = copy.deepcopy(config)
...
def test_hidden_canonical_archive_member_fails() -> None:
    archive, probe, _, config = _fixture()
    del archive
    probe = copy.deepcopy(probe)
    config = copy.deepcopy(config)
    ...
    probe_bytes = _rehash_probe(probe, config)
    with pytest.raises(ObservationPinError):
        pin_observation(changed_archive, probe, probe_bytes, config)
