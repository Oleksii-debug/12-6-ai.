...
The parent readiness authority is pinned at head `05143c6109f0d3d2168f711f9acdee7af481218f`. It keeps real ~20M training blocked both for data readiness and for checkpoint-integrity retest: NEXT100-075 currently rejects 8 of 11 corruption classes before mutation, while 3 cases still require remediation and a full exact-production rerun.
...
2. Freeze a sorted record inventory and candidate identity with the parent `build_predecontam_candidate_identity.py` fail-closed contract.
...
7. Requalify ~20M checkpoint and optimizer mechanics on the frozen corpus contract; checkpoint integrity must independently clear all 11 corruption classes.
