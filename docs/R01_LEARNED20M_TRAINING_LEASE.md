...
`build_authorized_training_run_lease()` is the finalized composition entrypoint: it first requires the external terminal-authority assessment to be ready, then creates the **same** `TRAINING_RUN` lease bound to the exact finalized manifest hash. It still does not execute an optimizer step and does not prove global exclusivity.
