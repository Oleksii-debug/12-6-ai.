...
The plan binds the exact source-registry convergence head `9ad8f74b12a2e991b7934356a88dd9a1f6ff3f41` and its convergence config without modifying that worker's files.
...
- English: 11,426,415 bytes
- code: 6,551,445 bytes
- total: 32,809,767 bytes

The current work-package allocation is 33,200,000 gross bytes, giving 390,233 bytes of planning headroom above that buffered minimum.
...
Passing this controller means only `PASS_PLANNING_CONTRACT_ONLY`.
...
The adversarial suite covers arithmetic drift, false prospect credit, public-availability promotion, evaluation leakage, paid-compute/training claims, under-buffering, excessive package concentration, fake terminal execution evidence, deterministic plan identity, and premature release claims.
