...
The scanner is only a configured gate until its exact-head workflow completes successfully. A green scan means the maintained scanner found no configured leak on that tested checkout/history; it is not proof that no secret can exist outside the scanner's rules. Any real leaked credential must be rotated even if later removed from Git history.
...
- independent AUDIT-B retest of the secret-scan result and the exact integrated candidate/release surface.

These controls must be evaluated on the exact integrated candidate. Action pinning, a maintained history scan, and removal of a floating pip upgrade are risk reduction, not a supply-chain PASS.
