...
## Late-bound live registry

This successor is stacked directly on the late-checked DATA-287 external snapshot registry head `b0523ccbc4b957615aac849d476cfa851be87578`.

DATA-287 dedicated exact-head workflow run `32968622282` is `success`. The live registry identity is `917e9bc31b2fa040d25e807ae3c01aa2cce32420752a891caacfb6c830e6632c`.

The live registry contains four independent training families: `ua.rada.open-data.laws-texts`, `en.standardebooks.manual`, `github:encode/httpx`, and `github:psf/requests`. It still lists `python.cpython.documentation` only as an explicit historical exclusion because DATA-228 was terminally unsuccessful. That historical exclusion is preserved; it is not rewritten as an adverse rights finding.

The late concurrency scan through observed PR #452 found no second terminal authority for the same CPython documentation lineage. Concurrent MDN and Ukrainian government source work is distinct. The earlier NEXT100-037 PR #451 is superseded because it was stacked before this live registry movement.
...
The source family is `python.cpython.documentation`. Additional files, mirrors, forks, or translations from the same official CPython documentation lineage do not automatically create additional independent-family credit.
...
The retained DATA-228/D03 normalization identity is:
...
The candidate was compared against the late-bound DATA-287 four-family registry. Its exact raw and normalized SHA-256 identities do not collide with that inventory, and `python.cpython.documentation` is a distinct upstream lineage. If later composed into a corpus, this authority may contribute at most one independent family credit.

This source is a successor to the DATA-228 CPython tutorial candidate. DATA-228 already produced source-probe and compatible PSF-2.0 rights evidence, but its dedicated run failed during environment bootstrap before terminal admission. DATA-293 correctly preserved that state as `NOT_ADMITTED_EVIDENCE_NOT_MATERIALIZED` and explicitly recorded no adverse rights finding. NEXT100-037 terminalizes the exact existing candidate without changing upstream bytes, normalization, family identity, or license terms.

## Machine authority

`configs/data/next100_037_python_docs_source_authority_v1.json`

Authority identity SHA-256: `46a00dc70db690ae2b3c4495a75283e7e752bdccb1047d4318c2ebadfa392f0d`.

Validation is stdlib-only and network-free:

`python tools/validate_next100_037_python_docs_source_authority.py`

The validator fail-closes on authority self-hash drift, live-registry identity/run drift, upstream identity drift, file-set expansion, rights-purpose weakening, loss of redistribution obligations, quality/privacy bypass, duplicate accepted chunks, code/evaluation overlap, family-credit inflation, or truth-boundary weakening.
