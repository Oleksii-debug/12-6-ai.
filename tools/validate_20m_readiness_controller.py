...
EXPECTED_SOURCE_REGISTRY_HEAD = "7da63b7d85b65b1508ef5c7d73bfa8d56e718c9f"
EXPECTED_SOURCE_REGISTRY_ID = "934933896a4b3b01dd58cd18d13bcc36245913f83412c6b3f697c64dd03e4d4d"
...
def validate(value: dict[str, Any]) -> None:
    ...
    _require(d05["direct_production_module_rerun_required"] is True, "D05 production rerun is not required")

    remediation = authorities["d05_remediation_convergence"]
    _require(remediation["current_head_sha"] == EXPECTED_D05_REMEDIATION_HEAD, "unexpected current D05 remediation head")
    _require(remediation["previous_green_head_sha"] == EXPECTED_D05_PREVIOUS_GREEN_HEAD, "unexpected prior green D05 head")
    _require(remediation["previous_exact_head_green_run"] == 33005163523, "unexpected prior green D05 run")
    _require(remediation["current_exact_head_d05_run"] == 33005737951, "unexpected current D05 run")
    _require(remediation["current_exact_head_d05_status"] == "QUEUED_AT_REFRESH", "current D05 run was not queued at snapshot refresh")
    ...
    _require(remediation["state"].startswith("NONTERMINAL_"), "D05 remediation state is not explicitly nonterminal")
    ...
    _require(corpus["authorized_balanced_no_replay_capacity"] == 0, "blocked corpus unexpectedly authorizes no-replay capacity")

    source_registry = authorities["source_registry_convergence_candidate"]
    _require(source_registry["schema_version"] == "12-6.next100-063-terminal-source-registry.v2", "source-registry v2 is not bound")
    _require(source_registry["head_sha"] == EXPECTED_SOURCE_REGISTRY_HEAD, "unexpected source-registry candidate head")
    _require(source_registry["registry_identity_sha256"] == EXPECTED_SOURCE_REGISTRY_ID, "unexpected source-registry identity")
    _require(source_registry["candidate_normalized_bytes"] == 266_476, "source-registry byte capacity changed")
    _require(source_registry["candidate_independent_family_count"] == 10, "source-registry family count changed")
    _require(source_registry["candidate_source_authority_count"] == 11, "source-registry authority count changed")
    _require(source_registry["family_minimum_gate"] == "PASS_PRE_GLOBAL_DEDUP", "source-registry family gate changed")
    _require(source_registry["draft"] is True, "source-registry candidate is no longer marked draft")
    _require(source_registry["terminal_authority"] is False, "source-registry candidate is prematurely terminal")
    _require(source_registry["training_authorized_loss_positions"] == 0, "source-registry candidate unexpectedly authorizes training loss positions")
    _require(source_registry["research_corpus_v1_target_gap_normalized_bytes"] == 19_733_524, "source-registry target gap changed")
    _require(source_registry["supersedes_registry_identity_sha256"] == "77fb69c558df8c59fdae00583c955c62ad088cda98fd16b335eedb26fb2d7526", "source-registry v1 supersession boundary changed")
    _require(
        all(row["family_count"] >= 2 for row in source_registry["pre_global_dedup_by_stratum"].values()),
    ...
    _require(ua["authority_state"] == "TERMINAL_RIGHTS_AND_IMMUTABLE_SNAPSHOT", "UA source lacks terminal rights/snapshot authority")
    _require(ua["training_selection"] == "BLOCKED_UNTIL_STANDARD_NEAR_MATCH_DECONTAMINATION", "UA source decontamination boundary changed")
    ...
    _require(decontam["state"] == "BLOCKED_NO_EXACT_CANDIDATE_CORPUS_IDENTITY", "decontamination blocker state changed")
    ...
    _require(selection["state"] == "NONEMPTY_SELECTION_AUTHORITY", "selection validation is not a nonempty authority")
    ...
    _require(gates["checkpoint_recovery_20m"] == "BLOCKED_D05_CORRUPTION_RETEST_REQUIRED", "20M checkpoint recovery gate changed")
    _require(gates["checkpoint_integrity_20m"] == "BLOCKED_PENDING_CURRENT_HEAD_GREEN_AND_11_OF_11_RERUN", "20M checkpoint integrity gate changed")
    _require(gates["source_registry_convergence"] == "CANDIDATE_V2_FAIL_CLOSED_266476_BYTES_10_FAMILIES", "source-registry convergence gate changed")
    _require(gates["exact_candidate_corpus_identity"] == "BLOCKED_NOT_MATERIALIZED", "candidate corpus identity gate changed")
    _require(gates["real_20m_training"] == "BLOCKED", "real 20M training is not blocked")
    _require(gates["unique_no_replay_loss_capacity"] == "BLOCKED_ZERO", "unique no-replay capacity is not blocked at zero")

    parallel_tracks = value["parallel_local_free_tracks"]
    _require("TERMINALIZE_NEXT100_063_AND_MATERIALIZE_EXACT_CANDIDATE_CORPUS_IDENTITY" in parallel_tracks, "source-registry/corpus convergence track is missing")
    _require("WAIT_FOR_PR504_CURRENT_EXACT_HEAD_D05_AND_THEN_RERUN_NEXT100_075_11_CASE_MATRIX" in parallel_tracks, "D05 convergence track is missing")
    ...
    _require(next_campaign[0] == "TERMINALIZE_NEXT100_063_SOURCE_REGISTRY_CONVERGENCE", "first ordered next action changed")
    _require(next_campaign[1] == "MATERIALIZE_EXACT_PRE_DECONTAMINATION_CANDIDATE_RECORD_INVENTORY_AND_IDENTITY", "second ordered next action changed")
    _require(next_campaign[-1] == "REQUEST_EXPLICIT_COMPUTE_AUTHORIZATION_ONLY_AFTER_CAMPAIGN_IS_DATA_READY", "compute authorization is not the final ordered action")
    _require(len(next_campaign) == len(set(next_campaign)), "ordered next actions contain duplicates")

    decision = value["terminal_decision"]
    _require(decision["status"] == "BLOCK_LONG_TRAINING_CONTINUE_LOCAL_FREE_ENGINEERING", "terminal decision is not fail-closed")
    _require(decision["primary_blocker"] == "RESEARCH_CORPUS_V1_NOT_TERMINAL_AND_ZERO_AUTHORIZED_UNIQUE_LOSS_CAPACITY", "primary blocker changed unexpectedly")
    _require("SOURCE_REGISTRY_V2_VECTOR_EXISTS_BUT_CORPUS_NOT_YET_MATERIALIZED" in decision["secondary_blockers"], "source-registry v2 blocker is missing")
    _require("D05_REMEDIATION_CURRENT_HEAD_NOT_YET_GREEN" in decision["secondary_blockers"], "D05 current-head blocker is missing")
    _require("NEXT100_075_FULL_11_CASE_PRODUCTION_RERUN_NOT_YET_GREEN" in decision["secondary_blockers"], "D05 11-case rerun blocker is missing")

    _require(canonical_identity(value) == value["evidence_identity_sha256"], "readiness evidence identity mismatch")
