#!/usr/bin/env python3
"""Materialize EVAL-647 immutable objects without persisting source payloads."""
...
def _raw_url(record: dict[str, Any], path: str | None = None) -> str:
    source_path = record["path"] if path is None else path
    return (
        "https://raw.githubusercontent.com/"
        f"{record['repository']}/{record['revision']}/{source_path}"
    )
...
def materialize(manifest: dict[str, Any], *, timeout: int = 30) -> dict[str, Any]:
    ...
    _require(
        reservation["final_test_payload_access_allowed"] is False,
        "final-test payload access widened",
    )
    _require(
        reservation["final_test_outcome_access_allowed"] is False,
        "final-test outcome access widened",
    )
    _require(reservation["training_allowed"] is False, "training accidentally allowed")
    _require(
        reservation["tokenizer_fit_allowed"] is False,
        "tokenizer fitting accidentally allowed",
    )
    _require(
        reservation["permanent_future_training_exclusion"] is True,
        "future exclusion missing",
    )

    objects = manifest["objects"]
    minimum = reservation["minimum_independent_families"]
    _require(len(objects) >= minimum, "insufficient reserved objects")
    families = {row["source_family"] for row in objects}
    _require(len(families) >= minimum, "insufficient independent families")

    sealed: list[dict[str, Any]] = []
    for row in objects:
        _require(row["training_allowed"] is False, f"training allowed for {row['repository']}")
        _require(
            row["tokenizer_fit_allowed"] is False,
            f"tokenizer fit allowed for {row['repository']}",
        )
        _require(
            row["permanent_future_training_exclusion"] is True,
        ...
        _require(
            observed_blob == row["git_blob_sha1"],
            f"Git blob identity drift for {row['repository']}:{row['path']}",
        )
        payload.decode("utf-8", errors="strict")
        observed_raw_sha = hashlib.sha256(payload).hexdigest()
        expected_raw_sha = row.get("raw_sha256")
        ...
        sealed.append(
            {
                ...
                "license_raw_sha256": hashlib.sha256(license_payload).hexdigest(),
                "license_git_blob_sha1": _git_blob_sha1(license_payload),
