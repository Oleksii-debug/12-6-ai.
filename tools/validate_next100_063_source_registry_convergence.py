...
EXPECTED_BASE = {
    ...
    "numeric_capacity_bytes": {"uk": 90044, "en": 84793, "code": 69133, "total": 243970},
    "independent_family_counts": {"uk": 2, "en": 1, "code": 4, "total": 7},
...
EXPECTED_LATE = {
    ...
    "NEXT100-038-DATA-EN-MDN": {
        "pr": 445,
        "head_sha": "902eccc0b3efff09a38dc89cda789180b6c6e754",
        "workflow_run": 32998544359,
        "workflow_name": "NEXT100-038 MDN Source Authority",
        "workflow_conclusion": "success",
        "authority_path": "evidence/next100_038/mdn_source_authority.v1.json",
        "authority_blob_sha1": "5b55a5b041381d3f9aa810b0a305f65d58e401bd",
    ...
    "NEXT100-037-DATA-EN-PYTHON-DOCS": {
        ...
        "training_authorized": True,
        "evaluation_authorized": False,
        "capacity_credit_status": "BLOCKED_ACCEPTED_CHUNK_BYTE_LEDGER_NOT_MATERIALIZED",
...
def validate(data: dict[str, Any]) -> dict[str, Any]:
    ...
    base_capacity = base["numeric_capacity_bytes"]
    base_families = base["independent_family_counts"]
    _require(_sum_total(base_capacity) == base_capacity["total"], "base capacity arithmetic mismatch")
    _require(_sum_total(base_families) == base_families["total"], "base family arithmetic mismatch")

    policy = data.get("credit_policy")
    _require(isinstance(policy, dict), "credit_policy missing")
    _require(
        "dedicated source workflow run id and exact workflow name" in policy.get("workflow_binding_rule", ""),
        "dedicated workflow binding rule missing",
    )
    _require(policy.get("failed_dedicated_workflow_credit") == 0, "failed workflow credit must be zero")
    _require(policy.get("queued_retest_probe_pr_text_only_credit") == 0, "nonterminal credit must be zero")
    ...
    _require(
        vector.get("canonical_balance_diversity_status") == "RETEST_REQUIRED_AFTER_SUCCESSOR_GLOBAL_DEDUP",
