...
def validate(data: dict[str, Any]) -> dict[str, Any]:
    ...
    _require(data.get("issue") == ISSUE, "issue binding changed")
    for key, expected in (
    ...
    _require(
        "dedicated source workflow run id and exact workflow name"
        in policy.get("workflow_binding_rule", ""),
        "dedicated workflow binding rule missing",
    )
    _require(
        policy.get("queued_retest_probe_pr_text_only_credit") == 0,
        "nonterminal source work received credit",
    )
    for key in (
        "evaluation_permission_inferred_from_training",
        "cross_source_dedup_inferred",
        "corpus_identity_inferred",
    ):
        _require(policy.get(key) is False, f"unsafe policy drift: {key}")
