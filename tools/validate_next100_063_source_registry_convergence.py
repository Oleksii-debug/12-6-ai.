...
def validate(data: dict[str, Any]) -> dict[str, Any]:
    ...
    _require(isinstance(base_capacity, dict) and isinstance(base_families, dict), "base vectors missing")
    _require(_sum_total(base_capacity) == base_capacity.get("total") == 243970, "base capacity arithmetic mismatch")
    _require(_sum_total(base_families) == base_families.get("total") == 7, "base family arithmetic mismatch")
    _require(base.get("source_object_count") == 11, "base source-object count changed")

    late = data.get("late_authorities")
    _require(isinstance(late, list) and len(late) == len(EXPECTED_LATE), "late authority set changed")
    by_worker: dict[str, dict[str, Any]] = {}
    for row in late:
        _require(isinstance(row, dict), "late authority row must be object")
        worker = row.get("worker_id")
        _require(isinstance(worker, str) and worker not in by_worker, "late worker ids must be unique")
    ...
    for worker, expected in EXPECTED_LATE.items():
        ...
        _require(row.get("workflow_conclusion") == "success", f"{worker}: workflow is not terminal-success")
        _require(row.get("terminal_status") == "ADMIT", f"{worker}: source is not ADMIT")
        _require(row.get("training_authorized") is True, f"{worker}: training permission missing")
        _require(row.get("evaluation_authorized") is False, f"{worker}: evaluation permission must not be inferred")
        ...
        _require(capacity >= 0 and family_credit in (0, 1) and object_count >= 0, f"{worker}: invalid credit")
        if worker == "NEXT100-037-DATA-EN-PYTHON-DOCS":
            _require(row.get("capacity_credit_status") == "BLOCKED_ACCEPTED_CHUNK_BYTE_LEDGER_NOT_MATERIALIZED", "CPython docs must fail closed on exact accepted-byte capacity")
            _require(row.get("source_normalized_bytes_not_capacity_credit") == 17901, "CPython source normalized bytes changed")
            _require(row.get("accepted_chunk_count") == 14 and row.get("rejected_chunk_count") == 2, "CPython quality partition changed")
            _require(capacity == family_credit == object_count == 0, "CPython docs received premature numeric credit")
        add_capacity[row["stratum"]] += capacity
        add_families[row["stratum"]] += family_credit
        add_objects += object_count

    expected_capacity = {key: int(base_capacity[key]) + add_capacity[key] for key in STRATA}
    expected_capacity["total"] = _sum_total(expected_capacity)
    expected_families = {key: int(base_families[key]) + add_families[key] for key in STRATA}
    ...
    _require(vector.get("numeric_capacity_bytes") == expected_capacity, "converged capacity arithmetic mismatch")
    _require(vector.get("independent_family_counts") == expected_families, "converged family arithmetic mismatch")
    _require(vector.get("numeric_source_object_count") == expected_objects, "converged object count mismatch")
    minimum = int(vector.get("family_minimum_required_per_stratum", 0))
    _require(minimum == 2, "family minimum policy changed")
    _require(all(expected_families[key] >= minimum for key in STRATA), "pre-dedup family minimum does not pass")
    _require(vector.get("family_minimum_candidate_status") == "PASS_PRE_SUCCESSOR_GLOBAL_DEDUP", "family candidate status mismatch")
    _require(vector.get("canonical_balance_diversity_status") == "RETEST_REQUIRED_AFTER_SUCCESSOR_GLOBAL_DEDUP", "G09 was promoted prematurely")
    ...
    _require(isinstance(targets, dict) and isinstance(gaps, dict), "acquisition vectors missing")
    _require(_sum_total(targets) == targets.get("total") == 20000000, "frozen acquisition target changed")
    expected_gaps = {key: int(targets[key]) - expected_capacity[key] for key in STRATA}
    expected_gaps["total"] = _sum_total(expected_gaps)
    _require(gaps == expected_gaps, "remaining acquisition gap mismatch")
    _require("not optimized causal-target" in acquisition.get("note", ""), "byte/token truth boundary missing")

    gates = data.get("downstream_handoff")
    _require(isinstance(gates, list), "downstream handoff missing")
    statuses = {row.get("gate"): row.get("status") for row in gates if isinstance(row, dict)}
    required = {
    ...
    for key in ("canonical_registry_rewritten", "post_dedup_capacity_claimed", "research_corpus_v1_released", "learned_20m_checkpoint_claimed", "learned_100m_checkpoint_claimed"):
