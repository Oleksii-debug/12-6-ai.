...
EXPECTED_PARENT_HEAD = "05143c6109f0d3d2168f711f9acdee7af481218f"
...
def validate(value: dict[str, Any]) -> None:
    ...
    checkpoint = parent["checkpoint_integrity"]
    assert checkpoint["audit_head_sha"] == EXPECTED_D05_AUDIT_HEAD
    assert checkpoint["verdict"] == "RETEST_REQUIRED"
    assert checkpoint["required_cases"] == 11
    assert checkpoint["rejected_before_mutation"] == 8
    assert checkpoint["blocking_case_count"] == 3
    assert checkpoint["remediation_candidate_head_sha"] == EXPECTED_D05_REMEDIATION_HEAD
    assert checkpoint["remediation_terminal"] is False
