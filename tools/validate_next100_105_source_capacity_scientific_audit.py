#!/usr/bin/env python3
"""Validate the NEXT100-105 fail-closed source-capacity audit."""
...
EXPECTED_PARENT_HEAD = "958ebec0f7c9cb00238c7df70566cefd6b504d92"
...
EXPECTED_SCHEMA = "12-6.next100-105-source-capacity-scientific-audit.v1"


class CapacityScientificAuditError(ValueError):
    pass


def _fail(message: str) -> None:
    raise CapacityScientificAuditError(message)
...
def validate(document: dict[str, Any]) -> None:
    ...
    if parent.get("head_sha") != EXPECTED_PARENT_HEAD:
        _fail("parent exact head drift")
    ...
    if finding.get("rejected_reason") != {"pii_phone": 2}:
        _fail("CPython rejection reason drift")
    ...
    if boundary.get("corpus_identity_claimed") is not False:
        _fail("corpus identity firewall violated")
...
def main() -> int:
    path = Path("configs/data/next100_105_source_capacity_scientific_audit_v1.json")
    validate(json.loads(path.read_text(encoding="utf-8")))
    print("NEXT100-105 source-capacity scientific audit: PASS")
    return 0
