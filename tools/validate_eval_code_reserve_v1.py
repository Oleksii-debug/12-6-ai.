...
def validate_document(doc: dict[str, Any]) -> dict[str, Any]:
    _require(
        doc.get("schema_version") == "12-6.eval-code-reserve-v1.contract.v1",
        "schema drift",
    )
    ...
    _require(
        predecessor.get("worker_id") == "NEXT100-057-CODE-EVAL-SET-V2",
        "predecessor worker drift",
    )
    _require(
        predecessor.get("head_sha") == "6713fe972b875b8a516122bda347264fb4099b2b",
        "predecessor head drift",
    )

    reservation = doc.get("reservation", {})
    _require(
        reservation.get("effective_at_utc") == "2026-08-26T19:46:57Z",
        "reservation timestamp drift",
    )
    ...
    _require(
        reservation.get("final_test_payload_access_allowed") is False,
        "final-test payload boundary widened",
    )
    _require(
        reservation.get("final_test_outcome_access_allowed") is False,
        "final-test outcome boundary widened",
    )
    _require(reservation.get("training_allowed") is False, "training accidentally allowed")
    _require(
        reservation.get("tokenizer_fit_allowed") is False,
        "tokenizer fitting accidentally allowed",
    )
    _require(
        reservation.get("permanent_future_training_exclusion") is True,
        "future exclusion missing",
    )
    _require(
        reservation.get("historical_training_exposure_required") == 0,
        "historical training boundary drift",
    )
    _require(
        reservation.get("historical_tokenizer_fit_exposure_required") == 0,
        "historical tokenizer boundary drift",
    )
    _require(reservation.get("training_overlap_required") == 0, "overlap boundary drift")
    _require(
        reservation.get("raw_payload_persisted_in_repository") is False,
        "raw eval payload must not be persisted",
    )
    ...
    for observed, expected in zip(objects, EXPECTED, strict=True):
        ...
        raw_sha = observed.get("raw_sha256")
        _require(raw_sha is None, "raw SHA-256 must remain unset until live materialization")
        ...
        _require(
            observed.get("tokenizer_fit_allowed") is False,
            "object tokenizer fitting accidentally allowed",
        )
        _require(
            observed.get("permanent_future_training_exclusion") is True,
            "object future exclusion missing",
        )

    families = {row["source_family"] for row in objects}
    _require(len(families) == 2, "two independent upstream families required")
    _require(
        doc.get("terminal_status") == "RESERVED_PENDING_RAW_SHA256_AND_OVERLAP_AUDIT",
        "terminal status drift",
    )
    truth = doc.get("truth_boundary", {})
    _require(
        truth.get("selection_validation_records_authorized") == 0,
        "selection records prematurely authorized",
    )
