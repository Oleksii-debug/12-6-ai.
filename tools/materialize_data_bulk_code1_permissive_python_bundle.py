...
def load_contract(path: Path) -> dict[str, Any]:
    ...
    _require(
        config.get("worker_id") == "DATA-BULK-CODE-1-PERMISSIVE-PYTHON-BUNDLE",
        "worker drift",
    )
    ...
    _require(
        config.get("contract_identity_sha256") == EXPECTED_CONTRACT_IDENTITY,
        "contract identity declaration drift",
    )
    ...
    _require(
        parent.get("head_sha") == "a546a1cb1434bee7db09d8fc8eff6040491de8ff",
        "parent head drift",
    )
    _require(
        parent.get("package_id") == "code-permissive-python-implementation-pool",
        "parent package drift",
    )
    ...
    _require(
        rights.get("automatic_canonical_capacity_credit") is False,
        "automatic capacity promotion forbidden",
    )
    ...
    expected_sources = {
        "pallets/flask": (
            ...
            "BSD-3-Clause",
            "LICENSE.txt",
            "9d227a0cc43c3268d15722b763bd94ad298645a1",
        ),
        "pallets/click": (
            ...
            "BSD-3-Clause",
            "LICENSE.txt",
            "d12a849186982399c537c5b9a8fd77bf2edd5eab",
        ),
        "pallets/jinja": (
            ...
            "BSD-3-Clause",
            "LICENSE.txt",
            "c37cae49ec77ad6ebb25568c1605f1fee5313cfb",
        ),
        "pallets/werkzeug": (
            "0005c79e09bae5f4cc2bd8ccd468d7dafe24a455",
            "src/werkzeug",
            "BSD-3-Clause",
            "LICENSE.txt",
            "c37cae49ec77ad6ebb25568c1605f1fee5313cfb",
        ...
        "agronholm/anyio": (
            "ae250440c90020b030ba4e83cccc37e9a84512c5",
            "src/anyio",
            "MIT",
            "LICENSE",
            "104eebf5a3002fccdaceef3a4cb936173c1c2035",
        ),
        "pytest-dev/pytest": (
            "28549a5f6b82bc916bb2ec5cb9fbfffe9b79fc66",
            "src/_pytest",
            "MIT",
            "LICENSE",
            "c3f1657fce94589bd1ec7cead810639047f3d359",
        ),
    }
    ...
    for source in sources:
        _require(isinstance(source, dict), "source contract must be object")
        repo = source.get("repository")
        _require(repo in expected_sources, f"unexpected source: {repo}")
        _require(repo not in repositories, f"duplicate repository: {repo}")
        ...
        observed = (
            ...
            source.get("license_path"),
            source.get("license_git_blob_sha1"),
        )
    ...
    _require(
        truth.get("corpus_identity") is None and truth.get("shard_identity") is None,
        "corpus/shard identity fabricated",
    )
...
def _verify_license(repo_dir: Path, source: dict[str, Any]) -> dict[str, Any]:
    ...
    _require(
        ...
        f"license missing or symlinked: {source['repository']}",
    )
    ...
    if source["license_id"] == "BSD-3-Clause":
        _require(
            ...
            "BSD license text missing grant",
        )
        _require(
            "Neither the name of the copyright holder" in text,
            "BSD third condition missing",
        )
    elif source["license_id"] == "MIT":
        _require(
            "The MIT License" in text and "Permission is hereby granted, free of charge" in text,
            "MIT license text mismatch",
        )
    else:
...
def _generated_marker(raw: bytes) -> str | None:
...
def _eligible_file_record(
    ...
    excluded_parts = {str(part).lower() for part in policy["exclude_directory_components"]}
    ...
    if hits:
        raise MaterializationError(
            f"high-confidence credential/secret pattern in {rel}: {','.join(sorted(hits))}"
        )

    marker = _generated_marker(raw)
    if marker is not None:
        return None, {"path": rel, "reason": "generated_marker", "marker": marker}
...
def materialize(config: dict[str, Any], workspace: Path) -> dict[str, Any]:
    ...
    for source in config["sources"]:
        ...
        _require(
            ...
            f"source root missing or symlinked: {source['repository']}",
        )
    ...
    report_core = {
        ...
        "security": {
            ...
            "credential_patterns": sorted(CREDENTIAL_PATTERNS),
        },
        "purpose_firewall": {
...
def validate_report(config: dict[str, Any], report: dict[str, Any]) -> None:
    ...
    _require(
        report.get("contract_identity_sha256") == EXPECTED_CONTRACT_IDENTITY,
    ...
    _require(isinstance(sources, list) and len(sources) == 6, "source report cardinality mismatch")

    expected_by_repo = {source["repository"]: source for source in config["sources"]}
    _require(
        {source.get("repository") for source in sources} == set(expected_by_repo),
        "source report repository set mismatch",
    )
    ...
    file_sum = 0
    for source in sources:
        expected = expected_by_repo[source["repository"]]
        _require(source["family_id"] == expected["family_id"], "family identity mismatch")
        _require(source["commit"] == expected["commit"], "source commit mismatch")
        _require(source["source_root"] == expected["source_root"], "source root mismatch")
        license_record = source["license"]
        _require(license_record["license_id"] == expected["license_id"], "license id mismatch")
        _require(license_record["license_path"] == expected["license_path"], "license path mismatch")
        _require(
            license_record["license_git_blob_sha1"] == expected["license_git_blob_sha1"],
            "license blob mismatch",
        )
        _require(
            source["eligible_file_count"] == len(source["files"]),
            "family file count mismatch",
        )
        _require(
            source["eligible_utf8_bytes"]
            == sum(row["utf8_bytes"] for row in source["files"]),
            "family byte count mismatch",
        )
        for exclusion in source.get("exclusions", []):
            _require(
                exclusion.get("reason") != "credential_pattern",
                "credential hit may not be represented as a harmless exclusion",
            )
    ...
    _require(
        security.get("credential_scan") == "PASS_NO_HIGH_CONFIDENCE_HITS",
        "credential scan is not terminal clean",
    )
    _require(
        security.get("credential_patterns") == sorted(CREDENTIAL_PATTERNS),
        "credential pattern evidence drift",
    )
    ...
    _require(
        firewall["automatic_canonical_capacity_credit"] is False,
        "automatic capacity promotion",
    )
    claim = report["claim_boundary"]
    _require(
        claim["corpus_identity"] is None and claim["shard_identity"] is None,
        "fabricated corpus/shard identity",
    )
    _require(claim["authorized_training_exposure"] == 0, "training exposure promoted")
    _require(
        claim["tokenizer_fit_authorized"] is False and claim["long_training_authorized"] is False,
        "training gate promoted",
    )
...
def main() -> int:
    ...
    try:
        ...
        if args.validate_report is not None:
            ...
            print(
                f"PASS report={report['report_identity_sha256']} "
                f"bytes={report['eligible_utf8_bytes']}"
            )
            return 0
        ...
        print(
            f"PASS report={report['report_identity_sha256']} "
            f"families={report['source_family_count']} "
            f"files={report['eligible_file_count']} "
            f"bytes={report['eligible_utf8_bytes']}"
        )
        return 0
    except (OSError, json.JSONDecodeError, MaterializationError, KeyError, TypeError, ValueError) as exc:
        print(f"DATA-BULK-CODE-1 FAIL: {exc}")
        return 1
