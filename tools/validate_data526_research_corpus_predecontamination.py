...
EXPECTED_DEPENDENCY = {
    ...
    "head_sha": "94dce83cbe611144f961b9f93b3be273345a7f62",
    "base_sha": "b0523ccbc4b957615aac849d476cfa851be87578",
    "candidate_registry_identity_sha256": "77fb69c558df8c59fdae00583c955c62ad088cda98fd16b335eedb26fb2d7526",
}
EXPECTED_DATA300 = {
...
def validate(data: dict[str, Any]) -> dict[str, Any]:
    ...
    _require(
        dependency.get("candidate_decision") == "CONVERGED_TERMINAL_SOURCE_VECTOR_PRE_GLOBAL_DEDUP_NOT_CORPUS_FREEZE",
        "candidate truth boundary changed",
    )
    _require(dependency.get("candidate_normalized_bytes") == 565743, "candidate bytes drift")
    _require(dependency.get("candidate_independent_family_count") == 13, "candidate family count drift")
    _require(dependency.get("candidate_source_authority_count") == 14, "candidate authority count drift")
    _require(
        dependency.get("candidate_by_stratum")
        == {
            "uk": {"normalized_bytes": 100856, "family_count": 4},
            "en": {"normalized_bytes": 168544, "family_count": 4},
            "code": {"normalized_bytes": 296343, "family_count": 5},
        },
    ...
    _require(ci.get("run_id") == 33005434232 and ci.get("name") == "CI", "generic CI binding changed")
    _require(
    ...
    _require(ci.get("accepted_as_terminal_authority") is False, "generic queued CI cannot authorize source convergence")
    ...
    _require(isinstance(requirements, list) and len(requirements) >= 5, "activation requirements incomplete")
    joined = "\n".join(str(item) for item in requirements)
    for term in ("terminal", "registry identity", "source object id", "content hash", "rights-purpose", "RETEST", "composite identity"):
    ...
    _require(
        handoff.get("current_next_action") == "WAIT_CANONICAL_NEXT100_063_TERMINAL_THEN_MATERIALIZE_PREDECONTAMINATION_MANIFEST",
        "next action changed",
    )
    _require(
