...
EXPECTED_IDENTITY = "1f7eebfd735ad7219a33ad82edd15192dda88281994e1488d40a13b57016df29"
EXPECTED_PARENT = {
...
EXPECTED_LATE = {
    449: ("40950a950b60921fd856af2719e1ae2486d9e892", 32997970539, "ua.kmu.portal.secretariat-news", 9153, 9153),
    462: ("d75edd497c7fb1054e86d892c9462f059c1f4aa9", 32998503672, "ua.verba.public-domain.nomis1864", 1659, 1659),
    445: ("902eccc0b3efff09a38dc89cda789180b6c6e754", 32998544359, "en.mdn.webdocs.prose", 6492, 6492),
    472: ("b7491745b34ac8679baaf69cb96cd609dcbe0a16", 32998703545, "en.usgov.nist.technical-series", 59358, 59358),
    467: ("5a6a495a24bce449334cbc5126d0114f61a9f57c", 32998356906, "python.cpython.documentation", 0, 17901),
}
EXPECTED_FAILED = {
...
def main() -> None:
    ...
    require(supersedes.get("v2_registry_identity_sha256") == "934933896a4b3b01dd58cd18d13bcc36245913f83412c6b3f697c64dd03e4d4d", "V2 binding drift")
    ...
    for row in rows:
        ...
        observed = (
            row.get("head"), row.get("dedicated_workflow_run"), row.get("family"),
    ...
    require(numeric_add == 76662, "late numeric byte arithmetic drift")
    require(numeric_total == 320632, "candidate numeric capacity drift")
    require(normalized_total == 338533, "normalized envelope drift")
    ...
    require(inv.get("candidate_independent_family_count") == len(families) == 12, "family count drift")
    ...
    require(inv.get("target_gap_numeric_training_capacity_bytes") == target - numeric_total == 19_679_368, "target gap drift")
