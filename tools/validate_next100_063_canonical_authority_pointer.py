...
EXPECTED_POINTER_ID = "603a330e775ca1678c5c62656593285cad202d7d458af24be02201837dc250d8"
...
def validate() -> dict[str, Any]:
    ...
    require(pointer.get("schema") == "12-6.next100-063-canonical-authority-pointer.v1", "pointer schema drift")
    require(pointer.get("worker_id") == "NEXT100-063-CANONICAL-AUTHORITY-SELECTOR", "pointer worker drift")
    require(pointer.get("repository") == "Oleksii-debug/12-6-ai.", "repository drift")
    require(pointer.get("pointer_identity_sha256") == EXPECTED_POINTER_ID, "pointer identity drift")
    require(identity_without_field(pointer, "pointer_identity_sha256") == EXPECTED_POINTER_ID, "pointer self-hash mismatch")
    ...
    require(canonical_path == ROOT / "configs/data/next100_063_terminal_source_registry_v2.json", "canonical path drift")
    require(canonical_path.is_file(), "canonical registry missing")
    registry = load(canonical_path)
    require(registry.get("schema_version") == canonical.get("schema_version"), "canonical schema mismatch")
    require(registry.get("worker_id") == canonical.get("worker_id"), "canonical worker mismatch")
    require(registry.get("registry_identity_sha256") == EXPECTED_CANONICAL_ID, "canonical registry identity drift")
    require(canonical.get("registry_identity_sha256") == EXPECTED_CANONICAL_ID, "pointer canonical identity drift")
    require(registry.get("pre_global_dedup_inventory", {}).get("candidate_normalized_bytes") == 303374, "canonical capacity drift")
    require(canonical.get("candidate_normalized_bytes") == 303374, "pointer capacity drift")
    require(registry.get("downstream_gate_vector", {}).get("authorized_balanced_no_replay_loss_positions") == 0, "training exposure promoted")
    require(canonical.get("authorized_balanced_no_replay_loss_positions") == 0, "pointer training exposure promoted")
    ...
    declared_forbidden = {
        row.get("observed_registry_identity_sha256") or row.get("registry_identity_sha256")
        for row in superseded
        if isinstance(row, dict)
    }
    require(declared_forbidden == FORBIDDEN_IDS, "superseded identity set drift")
    require(all("DO_NOT_CONSUME" in str(row.get("status", "")) for row in superseded), "superseded status weakened")

    require(
        pointer.get("consumer_rule")
        == "DOWNSTREAM_MUST_RESOLVE_THIS_POINTER_AND_MATCH_CANONICAL_REGISTRY_IDENTITY_BEFORE_CONSUMING_NEXT100_063",
        "consumer rule weakened",
    )
    boundary = pointer.get("truth_boundary")
    require(isinstance(boundary, dict) and boundary, "truth boundary missing")
    require(all(value is False for value in boundary.values()), "downstream claim prematurely promoted")
