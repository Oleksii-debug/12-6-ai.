"""LOCAL_FREE multilingual tokenizer, mixture, packing, and S2 probe."""
...
TRAIN = (
    ("uk-1", "Українська мова має відмінки, дієвідмінювання і словотвір. Ці дані "
     "потрібні для базового передтренування моделі.", "uk", "natural"),
    ("uk-2", "Дослідники працюють із текстами різних жанрів, щоб модель бачила слова "
     "у називному, родовому, давальному, знахідному та орудному відмінках.", "uk", "natural"),
    ("uk-3", "Київ, Львів і Ужгород мають різні мовні контексти; ґрунтовний корпус "
     "повинен містити літери ґ, ї, є, і та природні апострофи.", "uk", "natural"),
    ("en-1", "The training corpus contains English prose with varied syntax and vocabulary "
     "so the base model learns next-token statistics rather than instructions.", "en", "natural"),
    ("en-2", "These records test deterministic data selection, source provenance, "
     "deduplication, and restart behavior for a universal language model.", "en", "natural"),
    ("en-3", "Data quality includes valid encoding, stable normalization, explicit source "
     "rights, and strict separation from held-out evaluation material.", "en", "natural"),
    ...
    records = tuple(
        admit_for_pretraining(
            PretrainingRecord(
                ...
                source_version="v1",
                source_manifest_sha256=sha(f"manifest::{modality}"),
                split="train",
...
def probe_metrics(tokenizer, unknown_token_id: int | None = None) -> dict[str, object]:
    results = tuple(
        measure_probe(tokenizer, probe, unknown_token_id=unknown_token_id)
        for probe in PROBES
    )
    return {
...
def tokenizer_comparison(records: tuple) -> dict[str, object]:
    ...
    for algorithm in ("bpe", "unigram"):
        ...
        byte_tokens = int(byte["tokens"])
        observed = int(metrics["tokens"])
        costs = {}
        for d_model in (128, 320, 768):
            costs[str(d_model)] = asdict(tokenizer_cost(
                name=algorithm,
        ...
        algorithms[algorithm] = {
            ...
            "repeatable_artifact_identity": (
                first.artifact_identity.config_sha256
                == second.artifact_identity.config_sha256
            ),
            "heldout": metrics,
    ...
    return {
        "same_train_corpus_for_algorithms": True,
        "validation_or_probe_used_for_training": False,
        "byte": byte,
...
def mixture_probe(records: tuple) -> dict[str, object]:
    manifests = {
        name: sha(json.dumps(sorted(
            record.record_id for record in records if record.language == name
        ), separators=(",", ":")))
...
def s2_probe(records: tuple) -> dict[str, object]:
    packed = list(iter_packed_examples(
        tuple(TextRecord(r.record_id, r.normalized_text, "train") for r in records),
        ByteTokenizer(),
    ...
    return {
        ...
        "optimizer_step_changed_embedding": changed,
...
def run(source_sha: str) -> dict[str, object]:
    ...
    payload: dict[str, object] = {
        ...
        "authority": "PROJECT_AUTHORED_SYNTHETIC_MECHANICS_NOT_REPRESENTATIVE_CORPUS",
        ...
        "language_counts": {
            name: sum(record.language == name for record in records)
            for name in ("uk", "en", "code")
        },
        ...
        "decision": {
            ...
            "reason": "representative rights-approved multilingual corpus evidence is required",
        },
    }
    canonical = json.dumps(
        payload, sort_keys=True, separators=(",", ":"), ensure_ascii=False
    )
    payload["evidence_sha256"] = sha(canonical)
    return payload
...
def main() -> int:
    ...
    print(json.dumps({
        ...
        "mixture": evidence["mixture"],
        "s2_probe": evidence["s2_probe"],
    }, sort_keys=True))
    return 0
