...
capacity threshold or grants training/corpus authority. It verifies the exact
artifact/report/archive/member identities and emits a derived probe report that
is acceptable to the existing normalization boundary as a content-addressed
successor pin.
"""
...
def _validate_config(config: Mapping[str, Any]) -> None:
    ...
    if parent.get("pr") != 618:
        raise ObservationPinError("parent probe PR must remain #618")
    head = parent.get("head_sha")
    if not isinstance(head, str) or not re.fullmatch(r"[0-9a-f]{40}", head):
        raise ObservationPinError("parent head must be an exact commit SHA")
    for field in (
        "artifact_digest",
        "probe_report_sha256",
        ...
        if field == "artifact_digest":
            if not isinstance(value, str) or not re.fullmatch(
                ...
                raise ObservationPinError("artifact_digest must be sha256:<hex>")
        else:
            _require_sha256(value, f"parent_probe.{field}")
    for field in ("workflow_run_id", "artifact_id"):
        value = parent.get(field)
        if isinstance(value, bool) or not isinstance(value, int) or value <= 0:
            raise ObservationPinError(f"parent_probe.{field} must be positive")
...
def pin_observation(
    ...
    if probe.get("observation_mode") != expected["observation_mode"]:
        raise ObservationPinError("observation mode drift")
    ...
    # Deliberately preserve discovery_capacity_threshold=FAIL_BELOW_MINIMUM.
