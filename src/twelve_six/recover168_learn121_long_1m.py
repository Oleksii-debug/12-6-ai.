...
STOP_POLICY = {
    ...
    "rule": (
        "stop before the next milestone when current BPB is strictly greater than "
        "the best BPB at any earlier evaluated milestone"
    ),
...
def _protocol(repo: Path, source_sha: str, root: Path) -> dict[str, Any]:
    ...
    value: dict[str, Any] = {
        ...
        "semantic_incumbent": (
            "LEARN-121 recovered through accepted LEARN-122 mechanics without "
            "architecture search"
        ),
        "model": {
            "label": LABEL,
        ...
        "corpus": {
            ...
            "train_by_stratum": {
                s: manifest["by_split_stratum"][f"train:{s}"]["byte_tokens"]
                for s in ("uk", "en", "code")
            },
            "truth_boundary": manifest["truth_boundary"],
        },
        "packing": {
            "version": base.PACKING_VERSION,
            "sequence_length": base.SEQ,
        ...
        "evaluation": (
            "full DATA-25 validation NLL/BPB with UA/EN/code breakdown and state "
            "non-mutation"
        ),
        ...
        "unsupported_claims": [
            ...
            "real_world_corpus_representativeness",
        ],
    }
...
def preflight(repo: Path, source_sha: str) -> dict[str, Any]:
    ...
    train_tokens = retained["by_split"]["train"]["byte_tokens"]
    return {
        ...
        "planned_fraction_of_train_bytes": MILESTONES[-1] / train_tokens,
        "no_recycling_required_by_budget": MILESTONES[-1] < train_tokens,
        "stop_policy": STOP_POLICY,
    }
...
def _selector(state: dict[str, Any]) -> dict[str, Any]:
    ...
    for row in rows[1:]:
        ...
        eligible = row["milestone_requested_tokens"] >= int(
            STOP_POLICY["minimum_milestone_for_stop_tokens"]
        )
        decision = {
...
def advance(repo: Path, source_sha: str, root: Path, target_tokens: int) -> dict[str, Any]:
    ...
    if state_path.exists():
        ...
        if selector["stop_triggered"]:
            skipped = {
                "status": "SKIPPED_BY_PREDECLARED_OVERFIT_STOP",
    ...
    return {
        "status": "COMPLETED",
        "target_tokens": target_tokens,
        "latest_optimized_tokens": state["latest_optimized_tokens"],
        "selector": selector,
    }
...
def _curve(run_dir: Path) -> list[dict[str, Any]]:
    return [
        json.loads(line)
        for line in (run_dir / "train-curve.jsonl").read_text(encoding="utf-8").splitlines()
        if line.strip()
    ]
...
def _training_stats(curve: list[dict[str, Any]], step: int) -> dict[str, Any]:
    ...
    ratios = [
        float(row["update_to_parameter_ratio"])
        for row in rows
        if row.get("update_to_parameter_ratio") is not None
    ]
    return {
        ...
        "clip_threshold": 1.0,
        "clip_frequency": (
            sum(1 for value in grads if value > 1.0) / len(grads) if grads else None
        ),
...
def _logits_snapshot(checkpoint: Path) -> dict[str, Any]:
    ...
    for name, prompt in PROMPTS.items():
        ...
        logits = [float(value) for value in backend.next_token_logits(ids)]
        raw = struct.pack("<" + "f" * len(logits), *logits)
        top = sorted(range(len(logits)), key=lambda index: (-logits[index], index))[:8]
        outputs[name] = {
            "prompt": prompt,
...
def _fresh_verify(
    ...
    return {
        ...
        "evaluation_non_mutation_passed": (
            before == after and bool(evaluation["non_mutation_passed"])
        ),
        "generation": generation,
    }


def _memorization_controls(
    root: Path,
    ...
    consumed = {
        stratum: sum(int(row["tokens"]) for row in curve if row["stratum"] == stratum)
        for stratum in ("uk", "en", "code")
    }
    capacities = {
        stratum: int(manifest["by_split_stratum"][f"train:{stratum}"]["byte_tokens"])
        for stratum in ("uk", "en", "code")
    }
    corpora = {
        stratum: "\n".join(
            str(row["text"])
            for row in base._rows(root / "corpus-a", manifest, "train", stratum)
        )
        for stratum in ("uk", "en", "code")
    }
    scans = []
            ...
            scans.append(
                {
                    ...
                    "exact_generated_continuation_found_in_same_stratum_training_text": bool(
                        text and len(text) >= 16 and text in corpora[stratum]
                    ),
                }
            )
    return {
        ...
        "consumption_fraction_by_stratum": {
            stratum: consumed[stratum] / capacities[stratum] for stratum in consumed
        },
        "recycling_occurred": any(
            consumed[stratum] > capacities[stratum] for stratum in consumed
        ),
        "exact_generation_continuation_scan": scans,
        "any_exact_generation_continuation_match": any(
            row["exact_generated_continuation_found_in_same_stratum_training_text"]
            for row in scans
        ),
        "scope_note": (
            "Exact continuation scan is a narrow memorization control, not a general "
            "privacy or memorization guarantee."
        ),
    }
...
def finalize(repo: Path, source_sha: str, root: Path) -> dict[str, Any]:
    ...
    for row in rows:
        item = dict(row)
        item["training"] = (
            None if row["optimizer_step"] == 0 else _training_stats(curve, row["optimizer_step"])
        )
        by_milestone.append(item)
    best = selector["best"]
    final = selector["final_completed"]
    best_verify = _fresh_verify(
        repo,
        source_sha,
        root,
        best["checkpoint"],
        ...
        phase_throughput.append(
            {
                ...
                "wall_seconds": wall,
                "optimized_tokens_per_second": delta / wall if wall > 0 else None,
    ...
    reproduction = {
        ...
        "base_runner_parts": [
            "experiments/learn122/runner.00.part",
            "experiments/learn122/runner.01.part",
            "experiments/learn122/runner.02.part",
        ],
        "commands": [
            (
                "python -m twelve_six.recover168_learn121_long_1m prepare "
                f"--repo-root . --source-sha {source_sha} --output-dir recover168-evidence"
            ),
            *[
                (
                    "python -m twelve_six.recover168_learn121_long_1m advance "
                    f"--repo-root . --source-sha {source_sha} "
                    f"--output-dir recover168-evidence --target-tokens {milestone}"
                )
                for milestone in MILESTONES
            ],
            (
                "python -m twelve_six.recover168_learn121_long_1m finalize "
                f"--repo-root . --source-sha {source_sha} --output-dir recover168-evidence"
            ),
        ],
    }
    report: dict[str, Any] = {
        ...
        "real_process_termination_and_resume_passed": (
            len(state["fresh_process_resumes"]) >= 1
            and all(
                int(record["first_resumed_optimizer_step"])
                == int(record["loaded_optimizer_step"]) + 1
                for record in state["fresh_process_resumes"]
            )
        ),
        "compute": {
            "device": "cpu",
        ...
        "memorization_controls": _memorization_controls(
            root,
            manifest,
            curve,
            state["generations"],
        ...
        "claims": {
            "learned_base_artifact": float(final["bits_per_byte"])
            < float(rows[0]["bits_per_byte"]),
            "intelligence": None,
...
def main(argv: list[str] | None = None) -> int:
    ...
    common.add_argument(
        "--output-dir",
        type=Path,
        default=Path("recover168-evidence"),
    )
    ...
    if args.command == "preflight":
        result = preflight(args.repo_root.resolve(), args.source_sha)
