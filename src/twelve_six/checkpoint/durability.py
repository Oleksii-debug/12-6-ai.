...
def _require_existing_real_parent(destination: Path) -> os.stat_result:
    ...
    if stat.S_ISLNK(observed.st_mode) or not stat.S_ISDIR(observed.st_mode):
        raise OSError(
            errno.ENOTDIR,
            "checkpoint destination parent must be a real non-symlink directory",
            str(parent),
        )
    return observed
...
def fsync_parent_directory(path: str | Path) -> None:
    ...
    try:
        ...
        if not _same_object(before, opened):
            raise OSError("checkpoint parent changed while opening for fsync")
        os.fsync(fd)
    finally:
        os.close(fd)
...
def install(core: Any) -> None:
    ...
    def save_checkpoint(directory: str | Path, **kwargs: Any) -> dict[str, Any]:
        ...
        _require_existing_real_parent(destination)
        staging_root = Path(
