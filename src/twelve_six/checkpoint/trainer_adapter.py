...
def load_trainer_checkpoint(
    ...
    *,
    ...
    """Verify/decode one exact byte snapshot, then restore fresh D02 targets.

    The canonical identity checks, trainer-state preflight, model compatibility,
    RNG preflight, and actual restore all consume one decoded state snapshot. The
    source directory is never re-opened and model-scale tensors are not decoded
    twice between verification and mutation.
    """
    ...
    # No checkpoint byte is reopened or decoded after this point. Identity,
    # trainer compatibility, model tensor compatibility and supported RNG checks
    # all completed before the first live mutation.
