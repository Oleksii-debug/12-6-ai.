...
def load_trainer_checkpoint(
    ...
    *,
    ...
    Canonical binding, model compatibility, trainer-state preflight and RNG
    preflight all consume one decoded representation of the exact immutable byte
    snapshot. The source directory is never reopened and model-scale weights or
    optimizer tensors are not decoded a second time before application.
    """
    ...
    # Decode the immutable checkpoint snapshot exactly once. Everything below
    # either validates or applies this same in-memory representation.
    ...
    # All fail-closed checks complete before the first live mutation.
