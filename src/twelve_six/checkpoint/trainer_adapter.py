...
def load_trainer_checkpoint(
    ...
    *,
    ...
    """Verify, decode, preflight, and restore one exact D02 checkpoint snapshot.

    The directory is read once by :func:`prepare_checkpoint_load`, and the
    verified payload is decoded exactly once. Identity checks, trainer-state
    preflight, model materialization, RNG preflight, and final mutation all
    consume that same in-memory decode. This avoids model/optimizer-scale
    duplicate materialization on the 100M/1B scaling path while preserving the
    no-mutation-before-validation guarantee.
    """
    ...
    # No source byte is re-opened or decoded after this point. All integrity,
    # lineage, trainer-state, model-geometry, and supported RNG checks completed
