...
def load_trainer_checkpoint(
    ...
    *,
    ...
    """Verify, decode once, preflight, then restore fresh D02 targets.

    Run binding, model compatibility, trainer-state compatibility and RNG
    compatibility consume one immutable decoded snapshot. The source directory is
    never re-opened and SafeTensors payloads are never decoded a second time before
    model mutation. This keeps the fail-closed ordering while avoiding duplicate
    model/optimizer materialization as checkpoint size grows.
    """
    ...
    # Every integrity, identity and compatibility check completes before the first
    # target mutation. Reuse the same decoded objects for apply rather than calling
    # load_verified_checkpoint(), which would decode the SafeTensors payloads again.
