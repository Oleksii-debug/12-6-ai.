...
def load_trainer_checkpoint(
    ...
    *,
    ...
    """Verify, decode once, preflight all targets, then restore fresh D02 targets.

    The directory is snapshotted exactly once by :func:`prepare_checkpoint_load`.
    The verified payload is decoded exactly once and the same in-memory arrays and
    trainer/RNG state are used for compatibility preflight and mutation. This avoids
    materializing model and optimizer tensors twice on the resume path as model size
    grows, while preserving the no-mutation-before-validation contract.
    """
    ...
    # All byte, identity, trainer, model and RNG checks are complete before the
    # first live mutation. Apply the already-decoded snapshot directly instead
    # of routing through load_verified_checkpoint(), which would decode it again.
