...
D04_EXPOSURE_STATE_SCHEMA = "12-6.unique-loss-exposure-state.v2"
_D04_EXPOSURE_STATE_KEYS = frozenset(
    {
        "schema_version",
...
_D04_TRAINER_BINDING_KEYS = frozenset(
    {
...
_SIDECAR_PAYLOAD_KEYS = frozenset(
    {
        "schema",
        "generation",
        "checkpoint_manifest_sha256",
...
_RESUME_REFERENCE_KEYS = frozenset(
    {
        "schema",
        "directory",
        "file",
...
def _checkpoint_d04_data(manifest: Mapping[str, Any]) -> dict[str, str]:
    ...
    return {
        field: _require_sha256(data.get(field), f"checkpoint.data.{field}")
        for field in fields
    }
...
def _context(
    *, generation: int, checkpoint_path: Path, manifest: Mapping[str, Any]
    ...
    optimizer_step = _require_nonnegative_int(
        identity.get("step"), "checkpoint optimizer_step"
    )
    tokens_seen = _require_nonnegative_int(
        identity.get("tokens_seen"), "checkpoint tokens_seen"
    )
...
def _validate_exposure_state(
    ...
    *,
    ...
    if set(value) != _D04_EXPOSURE_STATE_KEYS:
        raise ResumeSidecarError("D04 exposure state fields do not match the V2 schema")
    if value.get("schema_version") != D04_EXPOSURE_STATE_SCHEMA:
        raise ResumeSidecarError("D04 exposure state schema mismatch")

    observed_state_hash = _require_sha256(
        value.get("state_identity_sha256"), "D04 exposure state identity"
    )
    state_without_hash = deepcopy(value)
    state_without_hash.pop("state_identity_sha256")
    if _d04_state_hash(state_without_hash) != observed_state_hash:
        raise ResumeSidecarError("D04 exposure state self-hash mismatch")
    ...
    for field in (
        ...
        if actual != d04_data[field]:
            raise ResumeSidecarError(
                f"D04 exposure state {field} mismatches checkpoint binding"
            )

    authorized_budget = _require_nonnegative_int(
        value.get("authorized_budget"), "D04 exposure authorized_budget"
    )
    one_pass_maximum = _require_nonnegative_int(
        value.get("one_pass_maximum"), "D04 exposure one_pass_maximum"
    )
    consumed = _require_nonnegative_int(
        value.get("consumed_loss_positions"),
        "D04 exposure consumed_loss_positions",
    )
    _require_nonnegative_int(
        value.get("claim_sequence"), "D04 exposure claim_sequence"
    )
    if authorized_budget > one_pass_maximum:
        raise ResumeSidecarError("D04 exposure authorized budget exceeds one-pass maximum")
    if consumed > authorized_budget:
        raise ResumeSidecarError("D04 exposure consumed positions exceed authorized budget")
    claims = value.get("claims")
    if not isinstance(claims, Mapping):
        raise ResumeSidecarError("D04 exposure claims must be a mapping")

    binding = value.get("trainer_state_binding")
    if not isinstance(binding, Mapping):
        raise ResumeSidecarError("D04 exposure trainer_state_binding is missing")
    if set(binding) != _D04_TRAINER_BINDING_KEYS:
        raise ResumeSidecarError(
            "D04 exposure trainer_state_binding fields do not match the V2 schema"
        )
    expected_binding = {
    ...
    if mismatches:
        raise ResumeSidecarError(
            f"D04 exposure/checkpoint binding mismatch: {mismatches}"
        )
    return value
...
def _write_payload_directory(
    ...
    staging = Path(
        tempfile.mkdtemp(prefix=f".{destination.name}.tmp-", dir=sidecar_root)
    )
    try:
        ...
        try:
            ...
            raise ResumeSidecarError(
                "resume sidecar destination appeared before publication"
            ) from exc
...
def validate_resume_reference(
    ...
    if set(reference) != _RESUME_REFERENCE_KEYS:
        raise ResumeSidecarError(
            "recovery resume sidecar reference fields do not match the V1 schema"
        )
    ...
    if reference.get("directory") != f"{SIDECAR_ROOT}/{generation_name}":
        raise ResumeSidecarError(
            "recovery resume sidecar directory/generation mismatch"
        )
...
def _read_payload(path: Path, expected_file_sha256: str) -> dict[str, Any]:
    ...
    if stat.S_ISLNK(before.st_mode) or not stat.S_ISREG(before.st_mode):
        raise ResumeSidecarError(
            "recovery resume sidecar must be a regular non-symlink file"
        )
    ...
    try:
        ...
        raise ResumeSidecarError(
            "recovery resume sidecar cannot be opened safely"
        ) from exc
    try:
        opened = os.fstat(fd)
        if not stat.S_ISREG(opened.st_mode):
            raise ResumeSidecarError(
                "recovery resume sidecar changed type while opening"
            )
    ...
    if not isinstance(value, dict):
        raise ResumeSidecarError(
            "recovery resume sidecar payload must be a JSON object"
        )
    return value
...
def load_resume_sidecar(
    ...
    *,
    ...
    if set(payload) != _SIDECAR_PAYLOAD_KEYS:
        raise ResumeSidecarError(
            "recovery resume sidecar payload fields do not match the V1 schema"
        )
    ...
    if supplied_hash != checked_reference["payload_sha256"]:
        raise ResumeSidecarError(
            "recovery resume sidecar pointer/payload hash mismatch"
        )
    ...
    checks = {
        ...
        "run_manifest_hash": (
            context.run_manifest_hash,
            payload.get("run_manifest_hash"),
        ),
