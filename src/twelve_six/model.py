...
class ModelSpec:
    """Architecture-only specification for a 12-6 Base decoder model."""
    ...
    def __post_init__(self) -> None:
        ...
        kv_heads = self.effective_n_kv_heads
        if kv_heads <= 0:
            raise ValueError("n_kv_heads must be a positive integer")
    ...
    @property
    def kv_dim(self) -> int:
        return self.effective_n_kv_heads * self.head_dim

    def to_dict(self) -> dict[str, Any]:
        payload = asdict(self)
        payload["n_kv_heads"] = self.effective_n_kv_heads
        return payload
    ...
    def parameter_breakdown(self) -> dict[str, int]:
        ...
        mlp_per_layer = 3 * self.d_model * self.d_ff
...
class StageConfig:
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "StageConfig":
        return cls(
            ...
            model=ModelSpec.from_dict(dict(payload["model"])),
        )
...
class CausalSelfAttention(nn.Module):
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.n_kv_heads = spec.effective_n_kv_heads
        self.head_dim = spec.head_dim
        self.dropout = spec.dropout

        self.q_proj = nn.Linear(spec.d_model, spec.d_model, bias=False)
        self.k_proj = nn.Linear(spec.d_model, spec.kv_dim, bias=False)
        self.v_proj = nn.Linear(spec.d_model, spec.kv_dim, bias=False)
        self.out_proj = nn.Linear(spec.d_model, spec.d_model, bias=False)
        self.rope = RotaryEmbedding(spec.head_dim, spec.rope_theta)
...
class TwelveSixDecoder(nn.Module):
    ...
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.final_norm = RMSNorm(spec.d_model, spec.norm_eps)
        self.lm_head = nn.Linear(spec.d_model, spec.vocab_size, bias=False)

        self.apply(self._init_module)
        residual_std = spec.init_std / math.sqrt(2.0 * spec.n_layers)
    ...
    def _init_module(self, module: nn.Module) -> None:
        if isinstance(module, (nn.Linear, nn.Embedding)):
            nn.init.normal_(module.weight, mean=0.0, std=self.spec.init_std)
