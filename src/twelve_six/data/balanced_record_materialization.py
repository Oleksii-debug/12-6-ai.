...
STRATA = ("ua", "en", "code")
_TARGET_STRATUM_BYTES = {"ua": 9_000_000, "en": 7_000_000, "code": 4_000_000}
...
def _sha256_obj(value: Any, *, trailing_lf: bool = True) -> str:
    payload = _canonical_json_bytes(value, trailing_lf=trailing_lf)
...
def _require_sha256(value: Any, field: str) -> str:
    if (
        ...
        raise BalancedMaterializationError(
            f"{field} must be exact lowercase SHA-256"
        )
    return value
...
class BalancedRecord:
    ...
    def __post_init__(self) -> None:
        ...
        if self.stratum not in STRATA:
            raise BalancedMaterializationError(
                f"unsupported stratum: {self.stratum!r}"
            )
        ...
        if self.stratum == "code" and self.modality != "code":
            raise BalancedMaterializationError(
                "code stratum requires code modality"
            )
        if self.stratum != "code" and self.modality != "text":
            raise BalancedMaterializationError(
                "ua/en strata require text modality"
            )
        _require_sha256(
        ...
        if (
            ...
            raise BalancedMaterializationError(
                f"{self.record_id}: source_bytes must equal positive normalized "
                "UTF-8 bytes"
            )
        if self.training_eligible is not True:
            raise BalancedMaterializationError(
                f"{self.record_id}: record is not training eligible"
            )
        if self.evaluation_reserved is not False:
            raise BalancedMaterializationError(
                f"{self.record_id}: evaluation-reserved record"
            )
        if self.quality_pass is not True:
            raise BalancedMaterializationError(
                f"{self.record_id}: quality gate is not PASS"
            )
        if self.privacy_pass is not True:
            raise BalancedMaterializationError(
                f"{self.record_id}: privacy gate is not PASS"
            )
...
def record_graph_identity(records: Sequence[BalancedRecord]) -> str:
    """Bind the complete post-policy record graph without raw text."""
    rows = sorted(
        (record.identity_mapping() for record in records),
...
def _normalize_stage_bindings(value: Mapping[str, str]) -> dict[str, str]:
    if set(value) != set(_REQUIRED_STAGE_BINDINGS):
        raise BalancedMaterializationError(
            "stage_bindings must contain exactly dedup, decontamination, "
...
def _normalize_stratum_totals(value: Any, field: str) -> dict[str, int]:
    if not isinstance(value, Mapping) or set(value) != set(STRATA):
        raise BalancedMaterializationError(
            f"{field} must contain exactly ua, en and code"
        )
    return {
        stratum: _require_nonnegative_int(value[stratum], f"{field}.{stratum}")
        for stratum in STRATA
    }


def _validate_balance_status(
    result: Mapping[str, Any],
    maximum: int,
    maximum_by_stratum: Mapping[str, int],
) -> str:
    status = result.get("status")
    allowed = {
        "TARGET_20M_SOURCE_MIX_FEASIBLE",
        "PARTIAL_MIX_FEASIBLE_ACQUIRE_MORE_DATA",
        "BLOCKED_NO_NONZERO_POLICY_COMPLIANT_MIXTURE",
    }
    if status not in allowed:
        raise BalancedMaterializationError("unsupported balance result status")
    if status == "TARGET_20M_SOURCE_MIX_FEASIBLE":
        if maximum != 20_000_000 or dict(maximum_by_stratum) != _TARGET_STRATUM_BYTES:
            raise BalancedMaterializationError(
                "TARGET_20M status does not bind the exact 45/35/20 target"
            )
    elif status == "PARTIAL_MIX_FEASIBLE_ACQUIRE_MORE_DATA":
        if not 0 < maximum < 20_000_000:
            raise BalancedMaterializationError(
                "partial balance status requires a nonzero sub-20M maximum"
            )
    elif maximum != 0 or any(maximum_by_stratum.values()):
        raise BalancedMaterializationError(
            "blocked balance status must have zero feasible bytes"
        )
    return status
...
def _validate_balance_result(
    ...
    if observed != expected:
        raise BalancedMaterializationError(
            "balance result does not match expected authority"
        )
    if result.get("target_total_source_bytes") != 20_000_000:
        raise BalancedMaterializationError("20M source-byte target drift")
    target_by_stratum = _normalize_stratum_totals(
        result.get("target_stratum_bytes"),
        "target_stratum_bytes",
    )
    if target_by_stratum != _TARGET_STRATUM_BYTES:
        raise BalancedMaterializationError("45/35/20 target stratum drift")

    claim = result.get("claim_boundary")
    if not isinstance(claim, Mapping):
        raise BalancedMaterializationError(
            "balance claim_boundary must be an object"
        )
    if claim.get("authorized_training_exposure_loss_positions") != 0:
        raise BalancedMaterializationError(
            "balance result already claims training exposure"
        )
    for field in (
        ...
        if claim.get(field) is not False:
            raise BalancedMaterializationError(
                f"balance claim boundary drift: {field}"
            )
    if claim.get("source_bytes_are_loss_positions") is not False:
        raise BalancedMaterializationError(
            "source bytes must not be relabelled as loss positions"
        )

    allocations = result.get("deterministic_maximum_allocation")
    if not isinstance(allocations, list):
        raise BalancedMaterializationError(
            "deterministic_maximum_allocation must be a list"
        )
    ...
    for index, raw in enumerate(allocations):
        if not isinstance(raw, Mapping):
            raise BalancedMaterializationError(
                f"allocation[{index}] must be an object"
            )
        family_id = _require_text(
            raw.get("family_id"),
            f"allocation[{index}].family_id",
        )
        if family_id in seen:
            raise BalancedMaterializationError(
                f"duplicate family allocation: {family_id}"
            )
        seen.add(family_id)
        stratum = raw.get("stratum")
        if stratum not in STRATA:
            raise BalancedMaterializationError(
                f"invalid allocation stratum: {stratum!r}"
            )
        allocated = _require_positive_int(
            raw.get("allocated_bytes"),
            "allocated_bytes",
        )
        ...
        if allocated > available or allocated > cap:
            raise BalancedMaterializationError(
                "family allocation exceeds capacity/cap"
            )
        normalized.append(
            {
                "family_id": family_id,
    ...
    maximum = _require_nonnegative_int(
        result.get("maximum_feasible_total_source_bytes"),
        "maximum_feasible_total_source_bytes",
    )
    if sum(row["allocated_bytes"] for row in normalized) != maximum:
        raise BalancedMaterializationError(
            "allocation total does not match maximum feasible total"
        )
    maximum_by_stratum = _normalize_stratum_totals(
        result.get("maximum_feasible_stratum_bytes"),
    ...
    if allocation_by_stratum != maximum_by_stratum:
        raise BalancedMaterializationError(
            "family allocations do not match maximum stratum totals"
        )
    status = _validate_balance_status(result, maximum, maximum_by_stratum)
    return observed, sorted(normalized, key=lambda row: row["family_id"]), status


def _selection_rank(balance_identity: str, record: BalancedRecord) -> str:
    payload = (
        f"{balance_identity}\0{record.family_id}\0{record.record_id}"
    ).encode("utf-8")
    return hashlib.sha256(payload).hexdigest()
...
def materialize_balanced_record_set(
    ...
    *,
    ...
    balance_identity, allocations, balance_status = _validate_balance_result(
        balance_result,
        expected_balance_result_identity_sha256,
    )
    ...
    if bindings["balance"] != balance_identity:
        raise BalancedMaterializationError(
            "stage_bindings.balance does not match result identity"
        )
    ...
    for allocation in allocations:
        ...
        if not family_records:
            raise BalancedMaterializationError(
                f"allocated family has no records: {family_id}"
            )
        if any(
            record.stratum != allocation["stratum"]
            for record in family_records
        ):
            raise BalancedMaterializationError(
                f"family stratum drift: {family_id}"
            )
        available = sum(record.source_bytes for record in family_records)
        if available != allocation["available_unique_bytes"]:
            raise BalancedMaterializationError(
                f"{family_id}: exact record bytes do not match "
                "balance available_unique_bytes"
            )
        ...
        ranked = sorted(
            family_records,
            key=lambda record: (
                _selection_rank(balance_identity, record),
                record.record_id,
            ),
        )
        for record in ranked:
            if record.source_bytes <= remaining:
                if record.dedup_cluster_id in selected_clusters:
                    raise BalancedMaterializationError(
                        "dedup cluster replay across selected records: "
                        f"{record.dedup_cluster_id}"
                    )
        ...
        family_rows.append(
            {
                **allocation,
                "selected_record_ids": sorted(
                    record.record_id for record in chosen
                ),
    ...
    total_gap = sum(
        row["whole_record_granularity_gap_bytes"] for row in family_rows
    )
    allocation_exact = total_gap == 0
    terminal_for_learned20 = (
        allocation_exact
        and balance_status == "TARGET_20M_SOURCE_MIX_FEASIBLE"
        and selected_total == 20_000_000
        and selected_by_stratum == _TARGET_STRATUM_BYTES
    )
    ...
    core: dict[str, Any] = {
        ...
        "selected_source_bytes_by_stratum": selected_by_stratum,
        ...
        _sha256_obj(
            {
                "materialization_identity_sha256": (
                    core["materialization_identity_sha256"]
                ),
                "selected_records": selected_rows,
            }
        )
