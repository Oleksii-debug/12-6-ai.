...
BASE_SCHEMA_VERSION = "12-6.next100-063-source-registry-convergence.v1"
...
def validate_plan(plan: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for index, package in enumerate(packages):
        ...
        if stage != "TERMINAL_ADMIT":
            ...
            nonterminal_authority = package.get("terminal_authority")
            _require(nonterminal_authority is None or nonterminal_authority is False, f"{package_id}: nonterminal package cannot carry terminal authority")
        else:
...
def validate_plan_against_base_authority(plan: Mapping[str, Any], base_path: Path) -> dict[str, Any]:
    """Cross-bind duplicated planner arithmetic to the exact parent authority artifact."""
    payload = base_path.read_bytes()
    base_config = json.loads(payload.decode("utf-8"))
    _require(base_config.get("schema_version") == BASE_SCHEMA_VERSION, "unexpected base authority schema")

    base = plan.get("base_authority")
    _require(isinstance(base, Mapping), "base_authority is required")
    actual_blob_sha1 = _git_blob_sha1(payload)
    _require(actual_blob_sha1 == base.get("config_blob_sha1"), "base authority config blob drift")

    claim = base_config.get("claim_boundary")
    _require(isinstance(claim, Mapping), "base authority claim_boundary is required")
    _require(claim.get("safe_result") == base.get("safe_result") == SAFE_BASE_RESULT, "base safe-result drift")

    converged = base_config.get("converged_pre_successor_dedup_vector")
    _require(isinstance(converged, Mapping), "base converged vector is required")
    base_credited = _byte_vector(converged.get("numeric_capacity_bytes"), "base.numeric_capacity_bytes")
    plan_credited = _byte_vector(plan.get("credited_pre_successor_dedup_bytes"), "credited_pre_successor_dedup_bytes")
    _require(base_credited == plan_credited, "planner credited vector drifted from bound base authority")

    acquisition = base_config.get("acquisition_plan_bytes")
    _require(isinstance(acquisition, Mapping), "base acquisition_plan_bytes is required")
    base_targets = _byte_vector(acquisition.get("frozen_targets"), "base.frozen_targets")
    base_gap = _byte_vector(acquisition.get("remaining_gap"), "base.remaining_gap")
    plan_targets = _byte_vector(plan.get("frozen_targets_bytes"), "frozen_targets_bytes")
    plan_gap = _byte_vector(plan.get("remaining_gap_bytes"), "remaining_gap_bytes")
    _require(base_targets == plan_targets, "planner frozen targets drifted from bound base authority")
    _require(base_gap == plan_gap, "planner remaining gap drifted from bound base authority")

    return {
        "status": "PASS_BASE_AUTHORITY_BINDING",
        "base_config_blob_sha1": actual_blob_sha1,
        "base_safe_result": SAFE_BASE_RESULT,
        "credited_pre_successor_dedup_bytes": base_credited,
        "remaining_gap_bytes": base_gap,
    }


def load_and_validate(path: Path, repo_root: Path | None = None) -> dict[str, Any]:
    plan = json.loads(path.read_text(encoding="utf-8"))
    report = validate_plan(plan)
    root = (repo_root or Path.cwd()).resolve()
    base_path = root / plan["base_authority"]["config_path"]
    _require(base_path.is_file(), f"bound base authority config is missing: {base_path}")
    report["base_authority_binding"] = validate_plan_against_base_authority(plan, base_path)
    return report
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    report = load_and_validate(args.plan, args.repo_root)
