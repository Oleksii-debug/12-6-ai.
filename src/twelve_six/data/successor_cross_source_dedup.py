...
This layer composes the terminal NEXT100-063 late authorities with the
NEXT100-065 V3 inventory without weakening the underlying fail-closed
...
def _verify_materialized_nist(row: Mapping[str, Any], raw_pdf: bytes, normalized: bytes) -> None:
    _require(row.get("materialization_policy") == NIST_POLICY, "unsupported NIST materialization policy")
    ...
    _require(
        _sha256(raw_pdf) == row.get("upstream_raw_sha256"),
        f"{row.get('source_id')}: upstream PDF SHA-256 drift",
    )
    _require(
        len(normalized) == row.get("expected_raw_bytes"),
        f"{row.get('source_id')}: normalized byte count drift",
    )
    _require(
...
def _materialize_nist(
    ...
    *,
    ...
    _verify_materialized_nist(row, raw_pdf, normalized)
    return normalized
...
def _validate_config(config: Mapping[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for raw in late:
        ...
        if row.get("materialization_policy") is not None:
            _require(row["materialization_policy"] == NIST_POLICY, f"{source_id}: unknown materialization policy")
            for key in ("upstream_raw_bytes", "upstream_raw_sha256", "pdf_start_page"):
...
def materialize_successor_payloads(
    ...
    *,
    ...
    for row in rows:
        ...
        if row.get("materialization_policy") == NIST_POLICY:
