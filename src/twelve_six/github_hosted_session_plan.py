...
_PROFILE_KEYS = {
    "schema_version", "profile_id", "provider", "portable_resource_provider",
    "resource_class", "runner_label", "maximum_cost_usd", "materially_paid",
    "configured_job_budget_minutes", "checkpoint_safety_margin_minutes",
    "max_session_work_minutes", "checkpoint_early_required",
    "fresh_process_resume_required", "content_addressed_checkpoint_required",
}
_PLAN_KEYS = {
    "schema_version", "plan_id", "profile_id", "profile_sha256",
    "estimated_runtime_minutes", "session_count", "sessions", "truth_boundary",
    "plan_sha256",
}
_SESSION_KEYS = {
    "session_index", "mode", "planned_work_minutes",
    "configured_job_budget_minutes", "checkpoint_deadline_minutes",
    "requires_previous_handoff",
}
_TRUTH_KEYS = {
    "authorized_optimized_target_exposure", "tokenizer_fit_authorized",
    "optimizer_updates_executed_on_real_targets", "training_executed",
    "learned_weights_created", "final_test_outcomes_read", "paid_compute_used",
}
_HANDOFF_KEYS = {
    "plan_sha256", "session_index", "run_id", "checkpoint_sha256",
    "checkpoint_manifest_sha256", "checkpoint_uri",
}
...
def _exact_keys(value: dict[str, Any], expected: set[str], prefix: str) -> list[str]:
    errors = [f"{prefix}_{key}_missing" for key in sorted(expected - set(value))]
    errors += [f"{prefix}_{key}_unexpected" for key in sorted(set(value) - expected)]
    return errors
...
def validate_profile(profile: Any) -> list[str]:
    ...
    if not _exact_int(profile.get("schema_version")) or profile.get(
        "schema_version"
    ...
    if all(_positive_int(item) for item in (job, margin, work)) and work > job - margin:
...
def build_session_plan(profile: Any, estimated_runtime_minutes: Any) -> dict[str, Any]:
        ...
        sessions.append(
            {
                ...
                "configured_job_budget_minutes": profile["configured_job_budget_minutes"],
...
def validate_session_plan(plan: Any, profile: Any) -> list[str]:
    ...
    if not _exact_int(plan.get("schema_version")) or plan.get(
        "schema_version"
    ) != PLAN_SCHEMA_VERSION:
    ...
    for index, session in enumerate(sessions, start=1):
        ...
        if not _exact_int(session.get("session_index")) or session.get(
            "session_index"
        ) != index:
            errors.append(f"session_{index}_index_mismatch")
        if session.get("mode") != ("FRESH_START" if index == 1 else "RESUME"):
        ...
        if not _positive_int(session.get("checkpoint_deadline_minutes")) or session.get(
    ...
    if any(
        row.get("planned_work_minutes") != profile["max_session_work_minutes"]
...
def validate_previous_handoff(
    ...
    if not _exact_int(handoff.get("session_index")) or handoff.get(
        "session_index"
...
def assess_session_launch(
    ...
    *,
    ...
    if not _positive_int(session_index):
        ...
        return SessionLaunchAssessment(False, None, tuple(sorted(set(blockers))), None)
    ...
    packet_hash = canonical_sha256(portable_packet) if isinstance(portable_packet, dict) else None
    ...
    if not _positive_int(limit) or limit > profile["configured_job_budget_minutes"]:
        blockers.append("portable_session_limit_exceeds_profile")
    safe_deadline = (
        profile["configured_job_budget_minutes"]
        - profile["checkpoint_safety_margin_minutes"]
    )
    if not _positive_int(first) or first > safe_deadline:
        blockers.append("portable_checkpoint_deadline_exceeds_safe_profile")
    ...
    if session_index == 1:
        ...
        if not portable.ready_for_initial_local_free_launch:
            blockers += [f"portable:{item}" for item in portable.launch_blockers]
    else:
        ...
        if not portable.ready_for_cross_provider_resume:
            blockers.append("canonical_same_provider_resume_authority_unavailable")
            blockers += [f"portable:{item}" for item in portable.resume_blockers]
        ...
        if not isinstance(lineage, dict):
            ...
            if lineage.get("source_provider") != profile["portable_resource_provider"]:
                blockers.append("same_provider_source_identity_mismatch")
            if lineage.get("parent_checkpoint_sha256") != previous_handoff.get(
                "checkpoint_sha256"
            ):
                blockers.append("parent_checkpoint_sha256_mismatch")
            if lineage.get("parent_manifest_sha256") != previous_handoff.get(
                "checkpoint_manifest_sha256"
            ):
                blockers.append("parent_manifest_sha256_mismatch")
            if lineage.get("previous_run_id") != previous_handoff.get("run_id"):
                blockers.append("previous_run_id_mismatch")
    blockers = sorted(set(blockers))
    return SessionLaunchAssessment(not blockers, session_index, tuple(blockers), packet_hash)
...
def main(argv: list[str] | None = None) -> int:
    ...
    plan_cmd.add_argument("--estimated-runtime-minutes", type=int, required=True)
    ...
    try:
        ...
        if args.command == "plan":
            result = build_session_plan(profile, args.estimated_runtime_minutes)
            print(json.dumps(result, ensure_ascii=False, sort_keys=True, indent=2))
            return 0
        errors = validate_session_plan(_read_json(args.plan), profile)
        print(json.dumps({"valid": not errors, "errors": errors}, sort_keys=True, indent=2))
        return 0 if not errors else 2
    except (OSError, json.JSONDecodeError, ValueError) as exc:
        print(json.dumps({"valid": False, "errors": [str(exc)]}, sort_keys=True, indent=2))
        return 2
