...
def load_s6_candidate(root: str | Path):  # type: ignore[no-untyped-def]
    root = Path(root).resolve()
    candidate = load_stage_config(root / CANDIDATE_PATH)
    ...
    _require(candidate.promotion_allowed is False, "S6 candidate cannot promote")
    _require(candidate.requires_preceding_stage_pass is True, "preceding-stage gate missing")
    return candidate
...
def load_s6_launch_profile(root: str | Path) -> dict[str, Any]:
    ...
    _require(payload.get("schema_version") == "12-6.s6-scale06-launch.v1", "launch schema drift")
    _require(payload.get("status") == "PREPARED_NOT_LAUNCHED", "launch status overclaim")
    ...
    _require(payload.get("candidate_model_identity_sha256") == MODEL_SHA256, "launch ModelSpec drift")
    ...
    _require(topology.get("minimum_cuda_memory_gib_per_gpu") == 24, "minimum GPU memory drift")
    _require(topology.get("distributed_strategy") == "fsdp2_full_shard", "distributed strategy drift")
    _require(topology.get("tensor_parallel_degree") == 1, "TP is not composed in SCALE-06 v1")

    training = payload.get("training")
    _require(isinstance(training, dict), "training profile missing")
    _require(training.get("precision") == "bf16_autocast_fp32_persistent_state", "precision drift")
    _require(training.get("sequence_length") == 4096, "sequence length drift")
    _require(training.get("microbatch_size") == 1, "microbatch drift")
    _require(training.get("gradient_accumulation_steps") == 16, "accumulation drift")
    _require(training.get("optimizer_steps_pilot") == 64, "pilot step count drift")
    _require(training.get("optimized_tokens_per_step") == 65_536, "token/update drift")
    _require(training.get("optimized_tokens_pilot") == 4_194_304, "pilot token count drift")
    _require(training.get("activation_checkpointing") == "blockwise_non_reentrant", "checkpointing drift")
    _require(training.get("attention_policy") == "flash_required", "attention policy drift")

    checkpoint = payload.get("checkpoint")
    _require(isinstance(checkpoint, dict), "checkpoint launch gate missing")
    _require(
        checkpoint.get("strategy") == "torch_distributed_checkpoint_sharded_required",
        "S6 requires sharded DCP",
    )
    _require(
        checkpoint.get("single_process_checkpoint_v1_allowed_for_paid_fsdp2_launch") is False,
    ...
    _require(data.get("status") == "BLOCKED_REAL_SCALE_ARTIFACTS_NOT_FROZEN", "data gate overclaim")
    _require(data.get("vocab_size_geometry_budget") == 32_768, "vocab budget drift")
    _require(data.get("s0_byte_fixture_allowed_for_capability_training") is False, "S0 fixture overclaim")
    _require(data.get("versioned_tokenizer_artifact_required") is True, "tokenizer artifact gate missing")
    _require(data.get("versioned_corpus_manifest_required") is True, "corpus manifest gate missing")
    _require(data.get("held_out_zero_optimization_split_required") is True, "held-out gate missing")
    ...
    _require(all(gates[name] is False for name in required_false), "S6 launch must remain blocked")
    return payload


def run_local_scale_analogue(seed: int = 20260825) -> dict[str, Any]:
    """Execute the inherited placement/checkpointing trainer seam at bounded LOCAL_FREE scale."""
    ...
    _require(grad_norm is not None and math.isfinite(float(grad_norm)), "analogue gradient non-finite")
...
def build_s6_readiness_report(
    ...
    *,
    ...
    """Build allocation-safe S6 evidence while preserving all launch blockers."""
    ...
    meta = build_meta_decoder(candidate.model, candidate.init, activation_checkpointing=True)
    ...
    _require(all(parameter.device.type == "meta" for parameter in meta.parameters()), "S6 meta allocation escaped")
...
def validate_s6_readiness_report(report: Mapping[str, Any]) -> None:
    ...
    _require(candidate.get("meta_instantiated_parameters") == PARAMETERS, "S6 meta count drift")
    _require(candidate.get("all_parameters_meta") is True, "S6 full allocation unexpectedly materialized")
    _require(candidate.get("model_identity_sha256") == MODEL_SHA256, "S6 model identity drift")
    _require(candidate.get("init_identity_sha256") == INIT_SHA256, "S6 init identity drift")
    for key in ("promotion_allowed", "architecture_frozen", "tokenizer_frozen"):
        _require(candidate.get(key) is False, f"prohibited candidate claim enabled: {key}")
    ...
    for world_size, persistent_total in expected_totals.items():
        ...
        _require(isinstance(estimate, Mapping), f"world-size {world_size} estimate missing")
        ...
        _require(estimate.get("full_training_checkpoint_bytes") == 11_989_278_720, "checkpoint byte drift")
        _require(estimate.get("weight_only_checkpoint_bytes") == 3_996_426_240, "weight byte drift")
        _require(estimate.get("kv_cache_bytes_per_token_per_sequence") == 36_864, "KV-cache byte drift")
        _require(estimate.get("estimated_activation_bytes_per_microbatch") == 797_966_336, "activation estimate drift")
        _require(estimate.get("estimated_training_flops_per_token") == 10_408_771_584, "FLOP estimate drift")
    ...
    _require(isinstance(blockers, list) and len(blockers) == 5, "S6 blocker set incomplete")
    _require(launch.get("compute_authorized") is False, "compute authorization fabricated")
    _require(launch.get("paid_compute_used") is False, "paid compute fabricated")
    _require(launch.get("cuda_execution_claimed") is False, "CUDA execution fabricated")
    _require(launch.get("distributed_execution_claimed") is False, "distributed execution fabricated")
    ...
    if analogue.get("status") == "PASS_LOCAL_FREE_ANALOGUE_ONLY":
        _require(analogue.get("optimizer_steps") == 1, "analogue optimizer step drift")
        _require(int(analogue.get("changed_embedding_elements", 0)) > 0, "analogue weight delta missing")
        _require(math.isfinite(float(analogue.get("loss"))), "analogue loss non-finite")
        _require(math.isfinite(float(analogue.get("grad_norm"))), "analogue grad norm non-finite")

    claimed_hash = report.get("report_sha256")
    _require(isinstance(claimed_hash, str) and len(claimed_hash) == 64, "report hash missing")
