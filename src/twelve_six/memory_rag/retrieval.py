...
class LexicalRetriever:
    """Deterministic BM25 lexical retrieval with stable memory_id tie breaking."""
    ...
    def _conflicts(items: Sequence[MemoryItem]) -> tuple[ConflictEvidence, ...]:
        ...
        for item in items:
            if item.claim_key is not None:
                grouped.setdefault(_normalize_claim(item.claim_key), []).append(item)
        conflicts: list[ConflictEvidence] = []
        for key in sorted(grouped):
            records = grouped[key]
            values = {
                _normalize_claim(record.claim_value)
    ...
    def retrieve(
        ...
        *,
        ...
        unique_query_tokens = tuple(dict.fromkeys(q_tokens))
        doc_freq = {
            token: sum(1 for doc in docs if token in doc) for token in unique_query_tokens
        }
